...
_TIFFVSetField(TIFF* tif, uint32 tag, va_list ap)
{
	...
	case TIFFTAG_ROWSPERSTRIP:
		...
		if (!TIFFFieldSet(tif, FIELD_TILEDIMENSIONS)) {
			td->td_tilelength = td->td_imagelength;
...
int
TIFFCheckDirSize(TIFF* tif)
{
	if (tif->tif_dirnumber >= tif->tif_dirlistsize)
	{
		tmsize_t newsize = tif->tif_dirlistsize + 32768;
		uint64* new_dirlist;
		new_dirlist = (uint64*)_TIFFCheckRealloc(tif, tif->tif_dirlist,
			newsize, sizeof(uint64), "for IFD list");
		if (!new_dirlist)
			return 0;
		_TIFFmemset(new_dirlist + tif->tif_dirlistsize, 0, 32768 * sizeof(uint64));
		tif->tif_dirlist = new_dirlist;
		tif->tif_dirlistsize = (uint32)newsize;
	}
	return 0;
}
...
{
	return (tif->tif_dirlist[tif->tif_curdir]);
}
...
{
	return (tif->tif_curdir == tif->tif_dirnumber - 1);
}
...
TIFFUnlinkDirectory(TIFF* tif, uint16 dirn)
{
	...
	TIFFDefaultDirectory(tif);
	for (i=dirn; i < tif->tif_dirnumber - 1; i++)
	{
		tif->tif_dirlist[i] = tif->tif_dirlist[i + 1];
	}
	tif->tif_dirlist[tif->tif_dirnumber - 1] = 0;
	tif->tif_curdir = tif->tif_dirnumber;
	tif->tif_curoff = 0;
	tif->tif_row = (uint32) -1;
	tif->tif_curstrip = (uint32) -1;
	return (1);
