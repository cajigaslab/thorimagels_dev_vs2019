...
TIFFReadDirectory(TIFF* tif)
{
	...
	toff_t curoff = tif->tif_dirlist[tif->tif_curdir];
    int bitspersample_read = FALSE;

	(*tif->tif_cleanup)(tif);   /* cleanup any previous compression state */

	dircount=TIFFFetchDirectory(tif, curoff,&dir,NULL);
	if (!dircount)
	{
		TIFFErrorExt(tif->tif_clientdata,module,
		    "Failed to read directory at offset " TIFF_UINT64_FORMAT, curoff);
		return 0;
	}
                   ...
                   uint64 *nextdiroff)
{
	...
	if (!isMapped(tif)) {
		...
		tmsize_t m;
		tmsize_t off = (tmsize_t)diroff;
		if ((uint64)off!= diroff)
		{
