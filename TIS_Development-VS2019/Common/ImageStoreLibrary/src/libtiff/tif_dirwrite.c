	...
	TIFFRewriteDirectory( TIFF *tif )
{
	...
	if( tif->tif_dirlist[tif->tif_curdir] == 0 )
	...
	*/
	...
	{
		if (tif->tif_header.classic.tiff_diroff == tif->tif_dirlist[tif->tif_curdir])
		{
			tif->tif_header.classic.tiff_diroff = 0;
			tif->tif_dirlist[tif->tif_curdir] = 0;
		...
		else
		{
			...
			while(1) {
				...
				if (nextnextdir==tif->tif_dirlist[tif->tif_curdir])
				{
					uint32 m;
					m=0;
					(void) TIFFSeekFile(tif,
	...
	{
		if (tif->tif_header.big.tiff_diroff == tif->tif_dirlist[tif->tif_curdir])
		{
	...
	TIFFWriteDirectorySec(TIFF* tif, int isimage, int imagedone, uint64* pdiroff)
{
	...
	*/
	...
	while (1)
	{
		...
		if (isimage)
		{
			if ((tif->tif_dirlist[tif->tif_curdir]==0)&&(!TIFFLinkDirectory(tif)))
				goto bad;
		}
		else
			tif->tif_dirlist[tif->tif_curdir] =(TIFFSeekFile(tif,0,SEEK_END)+1)&(~((toff_t)1));
		if (pdiroff!=NULL)
			*pdiroff=tif->tif_dirlist[tif->tif_curdir];
		if (!(tif->tif_flags&TIFF_BIGTIFF))
			dirsize=2+ndir*12+4;
		else
			dirsize=8+ndir*20+8;
		tif->tif_dataoff=tif->tif_dirlist[tif->tif_curdir] +dirsize;
		if (!(tif->tif_flags&TIFF_BIGTIFF))
			tif->tif_dataoff=(uint32)tif->tif_dataoff;
		if ((tif->tif_dataoff<tif->tif_dirlist[tif->tif_curdir])||(tif->tif_dataoff<(uint64)dirsize))
		{
	...
	if (isimage)
	{
		if (TIFFFieldSet(tif,FIELD_SUBIFD)&&(tif->tif_subifdoff==0))
		{
			...
			if (!(tif->tif_flags&TIFF_BIGTIFF))
				tif->tif_subifdoff=tif->tif_dirlist[tif->tif_curdir] +2+na*12+8;
			else
				tif->tif_subifdoff=tif->tif_dirlist[tif->tif_curdir] +8+na*20+12;
		}
	}
	...
	if (!(tif->tif_flags&TIFF_BIGTIFF))
	{
		...
		*(uint16*)n=(uint16)ndir;
		...
		nTmp = (uint32)tif->tif_dirlist[tif->tif_curdir + 1];
		if (tif->tif_flags&TIFF_SWAB)
			TIFFSwabLong(&nTmp);
		_TIFFmemcpy(n,&nTmp,4);
	}
	else
	{
		...
		*(uint64*)n=ndir;
		...
		_TIFFmemcpy(n, &tif->tif_dirlist[tif->tif_curdir + 1], 8);
		if (tif->tif_flags&TIFF_SWAB)
			TIFFSwabLong8((uint64*)n);
	}
	_TIFFfree(dir);
	dir=NULL;
	if (!SeekOK(tif,tif->tif_dirlist[tif->tif_curdir]))
	{
	...
	TIFFLinkDirectory(TIFF* tif)
{
	...
	*/
	if (tif->tif_flags & TIFF_INSUBIFD)
	{
		...
		{
			uint32 m;
			m = (uint32)tif->tif_dirlist[tif->tif_curdir];
			if (tif->tif_flags & TIFF_SWAB)
				TIFFSwabLong(&m);
		...
		{
			uint64 m;
			m = tif->tif_dirlist[tif->tif_curdir];
			if (tif->tif_flags & TIFF_SWAB)
				TIFFSwabLong8(&m);
			(void) TIFFSeekFile(tif, tif->tif_subifdoff, SEEK_SET);
			if (!WriteOK(tif, &m, 8)) {
	...
	if (!(tif->tif_flags&TIFF_BIGTIFF))
	{
		...
		if (tif->tif_curdir == 0) {
			...
			*/
			tif->tif_header.classic.tiff_diroff = (uint32)tif->tif_dirlist[tif->tif_curdir];
		...
		*/
		predir = (uint32)tif->tif_dirlist[tif->tif_curdir - 1];

		if (!SeekOK(tif, predir) ||
			!ReadOK(tif, &dircount, 2)) {
				TIFFErrorExt(tif->tif_clientdata, module,
					"Error fetching directory count");
				return (0);
	...
	else
	{
		...
		if (tif->tif_curdir == 0) {
			...
			* First directory, overwrite offset in header.
			*/
			tif->tif_header.big.tiff_diroff = tif->tif_dirlist[tif->tif_curdir];
			...
			* Not the first directory, search to the last and append.
			*/
			uint64 predir = tif->tif_dirlist[tif->tif_curdir - 1];
			uint64 dircount64;
			uint16 dircount;
			if (!SeekOK(tif, predir) ||
				!ReadOK(tif, &dircount64, 8)) {
	...
	tmsize_t count, void* data)
{
	...
	read_offset = tif->tif_dirlist[tif->tif_curdir];
