...
const char* const ExperimentXML::CAMERA_ATTR[NUM_CAMERA_ATTRIBUTES] = { "name","width","height","pixelSizeUM","exposureTimeMS","gain","blackLevel","lightmode","left","top","right","bottom","binningX","binningY","readoutTapIndex","tapBalance","readoutSpeedIndex","averageMode","averageNum","verticalFlip","horizontalFlip","imageAngle","channel", "colorImageTypeIndex", "polarImageTypeIndex", "isContinuousWhiteBalanceEnabled", "continuousWhiteBalanceNumFrames", "redGain", "greenGain", "blueGain"};

long ExperimentXML::GetImageArea(long cameraType, long lsmtype, long mROIEnabled, long& pixelX, long& pixelY, double& pixelSizeUM)
{
	if (_xmlObj.get() == NULL)
	{
	...
	else
	{
		...
		case ICamera::LSM:
		{
			...
			if (ICamera::LSMType::RESONANCE_GALVO_GALVO == lsmtype && TRUE == mROIEnabled)
			{
...
long ExperimentXML::GetCamera(string& name, long& width, long& height, double& pixelSizeUM, double& exposureTimeMS, long& gain, long& blackLevel, long& lightMode, long& left, long& top, long& right, long& bottom, long& binningX, long& binningY, long& tapsIndex, long& tapsBalance, long& readOutSpeedIndex, long& averageMode, long& averageNum, long& verticalFlip, long& horizontalFlip, long& imageAngle, long& channel, long& colorImageType, long& polarImageType, long& isContinuousWhiteBalance, long& continuousWhiteBalanceNumFrames, double& redGain, double& greenGain, double& blueGain)
{
	...
	for (index = 0; index < NUM_CAMERA_ATTRIBUTES; index++)
	{
		...
		case 24:
			...
			break;
		case 25:
			ss >> isContinuousWhiteBalance;
			break;
		case 26:
			ss >> continuousWhiteBalanceNumFrames;
			break;
		case 27:
			ss >> redGain;
			break;
...
long ExperimentXML::SetCamera(string name, long width, long height, double pixelSizeUM, double exposureTimeMS, long gain, long blackLevel, long lightMode, long left, long top, long right, long bottom, long binningX, long binningY, long tapsIndex, long tapsBalance, long readOutSpeedIndex, long averageMode, long averageNum, long verticalFlip, long horizontalFlip, long imageAngle, long channel, long colorImageType, long polarImageType, long isContinuousWhiteBalance, long continuousWhiteBalanceNumFrames, double redGain, double greenGain, double blueGain)
{
	string str;
	stringstream ss;
...
const char* const ExperimentXML::LSM_ATTR[NUM_LSM_ATTRIBUTES] = { "areaMode","areaAngle","scanMode","interleave","pixelX","pixelY","channel","fieldSize","offsetX","offsetY","averageMode",
	"averageNum","clockSource","inputRange1","inputRange2","twoWayAlignment","extClockRate","dwellTime","flybackCycles","inputRange3","inputRange4", "minimizeFlybackCycles", "polarity1", "polarity2", "polarity3", "polarity4", "verticalFlip", "horizontalFlip", "crsFrequencyHz", "timeBasedLineScan", "tbLineScanTimeMS", "ThreePhotonEnable", "NumberOfPlanes", "selectedImagingGG", "selectedStimGG", "pixelAspectRatioYScale"};
	...
	long& verticalFlip, long& horizontalFlip, double& crsFrequencyHz, long& timeBasedLineScan, long& timeBasedLSTimeMS, long& threePhotonEnable, long& numberOfPlanes, long& selectedImagingGG, long& selectedStimGG, double& pixelAspectRatioYScale)
{
	...
	for (index = 0; index < NUM_LSM_ATTRIBUTES; index++)
	{
		...
		case 32:
			...
			break;
		case 33:
			ss >> selectedImagingGG;
			break;
		case 34:
			ss >> selectedStimGG;
			break;
		case 35:
			ss >> pixelAspectRatioYScale;
			break;
		}
	}
	return TRUE;
...
long ExperimentXML::GetSubImages(vector<SubImage>& subImages, long cameraType, long lsmType, long mROIEnabled)
{
	if (_xmlObj.get() == NULL)
	{
	...
	else
	{
		...
		if (FALSE == GetImageArea(cameraType, lsmType, mROIEnabled, scanAreaPixelX, scanAreaPixelY, pixelSizeUM))
			return FALSE;
...
const char* const ExperimentXML::CAPTURESEQUENCE_ATTR[NUM_CAPTURESEQUENCE_ATTRIBUTES] = { "enable", "sequentialType"};

long ExperimentXML::GetCaptureSequence(long& enable, long& sequentialType)
{
	string str;
...
const char* const ExperimentXML::DIGITALIO = "DigitalIO";
...
const char* const ExperimentXML::SEQUENCESTEP_DIGITALIO_ATTR[NUM_SEQUENCESTEP_DIGITALIO_ATTRIBUTES] = { "enable", "digOut1", "digOut2", "digOut3", "digOut4", "digOut5", "digOut6", "digOut7", "digOut8" };

const char* const ExperimentXML::SEQUENCESTEP_CAMERA_ATTR[NUM_SEQUENCESTEP_CAMERA_ATTRIBUTES] = { "channel", "exposure" };

const char* const ExperimentXML::SEQUENCESTEP_EPITURRET_ATTR[NUM_SEQUENCESTEP_EPITURRET_ATTRIBUTES] = { "position", "positionName" };
	...
	else
	{
		...
		if (parent)
		{
			...
			for (ticpp::Element* step = parent->FirstChildElement(SEQUENCESTEP, false); step != 0; step = step->NextSiblingElement(SEQUENCESTEP, false))
			{
				...
				//retrieve channel step LSM Settings
				...
				//retrieve sequence step digitalIO Settings
				ticpp::Element* digitalIO = step->FirstChildElement(DIGITALIO, false);
				for (index = 0; index < NUM_SEQUENCESTEP_DIGITALIO_ATTRIBUTES; index++)
				{
					try
					{
					digitalIO->GetAttribute(SEQUENCESTEP_DIGITALIO_ATTR[index], &str);
					stringstream ss(str);

					switch (index)
					{
					case 0:	ss >> sequenceStep.DigSwitchesEnabled; break;
					case 1:	ss >> sequenceStep.DigSwitchPos[0]; break;
					case 2:	ss >> sequenceStep.DigSwitchPos[1]; break;
					case 3:	ss >> sequenceStep.DigSwitchPos[2]; break;
					case 4:	ss >> sequenceStep.DigSwitchPos[3]; break;
					case 5:	ss >> sequenceStep.DigSwitchPos[4]; break;
					case 6:	ss >> sequenceStep.DigSwitchPos[5]; break;
					case 7:	ss >> sequenceStep.DigSwitchPos[6]; break;
					case 8:	ss >> sequenceStep.DigSwitchPos[7]; break;
					}
					}
					catch (exception ex)
					{
						logDll->TLTraceEvent(ERROR_EVENT, 1, L"ExperimentManager could not read one of the DigitalIO attributes from the sequence");
					}
				}

				//retrieve channel step Camera Settings
				ticpp::Element* camera = step->FirstChildElement(CAMERA, false);
				for (index = 0; index < NUM_SEQUENCESTEP_CAMERA_ATTRIBUTES; index++)
				{
					try
					{
						camera->GetAttribute(SEQUENCESTEP_CAMERA_ATTR[index], &str);
						stringstream ss(str);

						switch (index)
						{
						case 0:	ss >> sequenceStep.CameraChannel; break;
						case 1:	ss >> sequenceStep.Exposure; break;
						}
					}
					catch (exception ex)
					{
						logDll->TLTraceEvent(ERROR_EVENT, 1, L"ExperimentManager could not read one of the Camera attributes from the sequence");
					}
				}

				//retrieve channel step Epi Turret Settings
				ticpp::Element* epi = step->FirstChildElement(EPITURRET, false);
				for (index = 0; index < NUM_SEQUENCESTEP_EPITURRET_ATTRIBUTES; index++)
				{
					try
					{
						epi->GetAttribute(SEQUENCESTEP_EPITURRET_ATTR[index], &str);
						stringstream ss(str);

						switch (index)
						{
						case 0:	ss >> sequenceStep.EpiTurretPosition; break;
						case 1: sequenceStep.EpiTurretPositionName = str; break;
						}
					}
					catch (exception ex)
					{
						logDll->TLTraceEvent(ERROR_EVENT, 1, L"ExperimentManager could not read one of the EpiTurret attributes from the sequence");
					}
				}
...
const char* const ExperimentXML::REMOTEFOCUS = "RemoteFocus";

const char* const ExperimentXML::REMOTE_FOCUS_ATTR[NUM_REMOTE_FOCUS_ATTRIBUTES] = { "steps", "startPlane", "stepSize", "captureMode", "customSequence", "IsRemoteFocus" };

long ExperimentXML::GetRemoteFocus(long& steps, long& startPlane, long& stepSize, long& captureMode, string& customSequence, long& isRemoteFocus)
{
	try
	{
		for (long index = 0; index < NUM_REMOTE_FOCUS_ATTRIBUTES; index++)
		{
			string str;
			if (!GetAttribute(REMOTEFOCUS, REMOTE_FOCUS_ATTR[index], str))
			{
				return FALSE;
			}
			...
			switch (index)
			{
			case 0:ss >> steps; break;
			case 1:ss >> startPlane; break;
			case 2:ss >> stepSize; break;
			case 3:ss >> captureMode; break;
			case 4:customSequence = str; break;
			case 5:ss >> isRemoteFocus; break;
			}
		}
	}
	catch (exception ex)
	{
		logDll->TLTraceEvent(ERROR_EVENT, 1, L"ExperimentManager could not read one of the RemoteFocus attributes");
	}

	return TRUE;
}

long ExperimentXML::SetRemoteFocus(long steps, long startPlane, long stepSize, long captureMode, string customSequence, long isRemoteFocus)
{
	string str;
	stringstream ss;
	...
	{
		getline(ss, str);
		if (!SetAttribute(REMOTEFOCUS, REMOTE_FOCUS_ATTR[index], str))
		{
			return FALSE;
		}
	}
...
long ExperimentXML::GetSampleRegions(long& viewMode, vector<SampleRegion>& sregions)
{
	...
	try
	{
		...
		if (configObj == NULL)
			return FALSE;
		const char* scanConfig = "ScanConfigure";
		ticpp::Element* templateLevel1 = configObj->FirstChildElement(TEMPLATE_NODES[0]);
		ticpp::Iterator<ticpp::Element> child1(templateLevel1, TEMPLATE_NODES[1]);
		...
		for (child1 = child1.begin(templateLevel1); child1 != child1.end(); child1++)
		{
			...
			ticpp::Element* templateLevel2 = child1->FirstChildElement(TEMPLATE_NODES[2]);
			ticpp::Iterator<ticpp::Element> child2(templateLevel2, TEMPLATE_NODES[3]);
			ticpp::Element* templateLevel2ScanConfig = child1->FirstChildElement(scanConfig);
			int stripeFieldSize = 0;
			templateLevel2ScanConfig->GetAttribute("StripeFieldSize", &stripeFieldSize);
			//get view mode and all scan area ids when area is enabled
			bool IsEnable = false;
			string str = "";
			for (child2 = child2.begin(templateLevel2); child2 != child2.end(); child2++)
			{
				...
				if (IsEnable)
				{
					...
					region.StripeFieldSize = stripeFieldSize;
