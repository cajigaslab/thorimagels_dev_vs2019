                    ...
                    if (command.Payload.Count > 0)
                    {
                        if (command.Payload[0].Equals("RunImmediately"))
                        {
                            if (command.Payload.Count > 1)
                            {
                                ...
                                if ((null != attrib) && (attrib.Value.Length > 0))
                                {
                                    string expTemplatesFldr = Path.GetDirectoryName(attrib.Value);
                                    string templateName = Path.GetFileNameWithoutExtension(attrib.Value);
                                    ...
                                    if (File.Exists(attrib.Value))
                                    {
                                        ...
                                        File.Copy(attrib.Value, pathActiveXML, true);
                                    }
                                    else
                                    {
                                        MessageBox.Show("Capture: Could not find file " + attrib.Value);
                                        return;
                                    }
                                    ...
                                    switch (((RunSampleLSViewModel)(this.MainWindowView).RunSampleLSView.DataContext).CurrentBleachMode)
                                    {
                                        ...
                                        case RunSampleLS.BleachMode.SLM:
                                            if (Directory.Exists(expSLMWaveforms))
                                            {
                                                ...
                                                {
                                                    Directory.CreateDirectory(dirPath.Replace(expSLMWaveforms, pathActiveSLMWaveforms));
                                                }
                                                //copy all files:
