...
#else
#define BLINK_SDK_API __declspec(dllimport)
#endif
...
public:
  ...
  ///                            arrives within this time, function returns (false).
  ///
  ...
  ///                            arrives within this time, function returns (false).
  ///
  /// @return @c true if there were no errors, otherwise @c false. When 
  ///         triggered, returns @true if asynchronous write was started 
  ///         successfully, @false if a previous write is in progress.
  /// @sa Get_last_error_message.
  // -------------------------------------------------------------------------
...
#if 0
  // -------------------------------------------------------------------------
  // Is_triggered_write_complete
  // -------------------------------------------------------------------------
  /// @param is_successful  All transient frames were downloaded without 
  ///                       errors.
  ///
  /// @return @c true if all frames from the previous Write_transient_frames
  ///         call were downloaded, and the result in @c is_successful is 
  ///         valid. Used with triggering only.
  // -------------------------------------------------------------------------
  bool Is_triggered_write_complete(bool *is_successful);
#endif
  ...
  /// @brief Calculates transient frames required to write a sequence specified
  ///        by the user-provided list
  ///
  /// @param target_images  List of pointers to target images.
  /// @param n_images       Number of images in list.
  ///
  /// @return @c true if there were no errors, otherwise @c false.
  /// @sa Get_last_error_message.
  // -------------------------------------------------------------------------
  bool Load_target_sequence(const unsigned char** target_images, 
                            const unsigned int n_images);


#if 0
  // -------------------------------------------------------------------------
  //  Start_overdrive_sequence
  /// @param board               Index of the board with the required SLM. The
  ///                            index is 1-based (not 0-based).
  /// @param wait_for_trigger    If supported by hardware, this enables use of
  ///                            an external trigger to load images to the SLM.
  /// @param external_pulse      Enables an external pulse on the last transient
  ///                            frame.
  /// @param trigger_timeout_ms  Time to wait for a trigger to arrive before
  ///                            aborting the sequence.
  /// @return @c true if there were no errors, otherwise @c false.
  /// @sa Get_last_error_message.
  // -------------------------------------------------------------------------
  bool Start_overdrive_sequence(int board,
                                bool wait_for_trigger, bool external_pulse,
