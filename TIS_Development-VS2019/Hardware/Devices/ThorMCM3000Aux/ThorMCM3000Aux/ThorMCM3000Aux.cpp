// ThorMCM3000Aux.cpp : Defines the exported functions for the DLL application.
//

#include "stdafx.h"
#include "Serial.h"
#include "ThorMCM3000Aux.h"
#include "ThorMCM3000AuxXML.h"
#include "Strsafe.h"
#include "ParamInfo.h"
#include <string>
#include <math.h>
...
const double ThorMCM3000Aux::MOTOR_COUNTER_COEFF[9] = { 0, 211.66666, 39.0625, 500, 500, 100.0, 5555.555, 211.66666, 11.90625};
...
//const double ThorMCM3000Aux::MOTOR_COUNTER_COEFF_OLDFIRMWARE[7] = { 0.0, 39.0625, 211.6667, 1.0, 500.0, 100.0, 1.0};


//Motor Types: No Motor, LNR, PLS, AScope Z, BScope, BScope Z, Bergamo Z High Torque (1 Count == MOTOR_COUNTER_COEFF[i]) ..... new
const double ThorMCM3000Aux::MOTOR_COUNTER_COEFF_OLDFIRMWARE[7] = { 0.0, 39.0625, 211.6667, 1.0, 500.0, 100.0, 100.0};
 

/// <summary>
/// Prevents a default instance of the <see cref="ThorMCM3000Aux"/> class from being created.
/// </summary>
ThorMCM3000Aux::ThorMCM3000Aux()
{
	_numOfAxes = 3;
	_xThreshold = 0.400;
	_yThreshold = 0.400;
...
/// <summary>
/// Finalizes an instance of the <see cref="ThorMCM3000Aux"/> class.
/// </summary>
ThorMCM3000Aux::~ThorMCM3000Aux()
{
...
bool ThorMCM3000Aux::_instanceFlag = false;

auto_ptr<ThorMCM3000Aux> ThorMCM3000Aux::_single (new ThorMCM3000Aux());

/// <summary>
/// Gets the instance.
/// </summary>
/// <returns>ThorMCM3000Aux *.</returns>
ThorMCM3000Aux* ThorMCM3000Aux::getInstance()
{
	if(!_instanceFlag)
	{
		_single.reset(new ThorMCM3000Aux());
		_instanceFlag = true;
		return _single.get();
	}
	else
...
long ThorMCM3000Aux::FindDevices(long &deviceCount)
{
	long ret = TRUE;
	...
	try
	{
		//Get portID, etc from hardware ThorMCM3000AuxSettings.xml
		auto_ptr<ThorMCM3000AuxXML> pSetup(new ThorMCM3000AuxXML());

		pSetup->GetConnection(portID,baudRate,address);
	}
	catch(...)
	{
		StringCbPrintfW(_errMsg,MSG_SIZE,L"Unable to locate ThorMCM3000AuxSettings.xml file");
		return FALSE;
	}

	if(FALSE == _serialPort.Open(portID, baudRate))
	{
		StringCbPrintfW(message,MSG_SIZE,L"ThorMCM3000Aux FindDevices could not open serial port");
		LogMessage(message);
		StringCbPrintfW(_errMsg,MSG_SIZE,L"ThorMCM3000Aux FindDevices could not open serial port or configuration file is not avaible.");
...
/// <returns>long.</returns>
long ThorMCM3000Aux::SelectDevice(const long device)
{
	long ret = FALSE;
	...
	switch(device)
	{
		case 0:
		{
			...
			{				
				...
				//Get portID, etc from  ThorMCM3000AuxSettings.xml
				pSetup->GetConnection(portID,baudRate,address);
				
				//get the min and max and threshold for the XStage, YStage, ZStage from ThorMCM3000AuxSettings.xml
				...
				//get the stage type for each motorAxis from ThorMCM3000AuxSettings.xml
			...
			if(FALSE == _serialPort.Open(portID, baudRate))
			{
				StringCbPrintfW(message,MSG_SIZE,L"ThorMCM3000Aux SelectDevice could not open serial port");
				LogMessage(message);
				StringCbPrintfW(_errMsg,MSG_SIZE,L"ThorMCM3000Aux SelectDevice could not open serial port or configuration file is not avaible.");
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::SetMotorParameters()
{
...
void ThorMCM3000Aux::LogMessage(wchar_t *message)
{
	#ifdef LOGGING_ENABLED
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::TeardownDevice()
{	
	_serialPort.Close();
...
/// <returns>long.</returns>
long ThorMCM3000Aux::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
{
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetDeviceParameters()
{
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetMotorType()
{
	Lock lock(critSect);
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetFirmwareVersion(double &firmwareversion)
{
	Lock lock(critSect);
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::QueryMotorTypeOldFirmware()
{
	Lock lock(critSect);
	for(int i=0; i < _numOfAxes; i++)
	{
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::QueryInitialMotorPos()
{
	Lock lock(critSect);
	for(int i=0; i < _numOfAxes; i++)
	{
...
/// <returns>long.</returns>
long ThorMCM3000Aux::SetParam(const long paramID, const double param)
{
	double value = param;
	if (NULL != _tableParams[paramID])
	{
		if(_tableParams[paramID]->GetParamID() == paramID)
		{
			...
			{
				StringCbPrintfW(message,MSG_SIZE,L"ThorMCM3000Aux SetParam failed. paramID: %d", paramID);
				LogMessage(message);
				return FALSE;
			}
		}
...
/// <param name="param">The parameter.</param>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetParam(const long paramID, double &param)
{
	double ret = -1;
	if (NULL != _tableParams[paramID])
	{
...
/// <returns>long.</returns>
long ThorMCM3000Aux::SetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <returns>long.</returns>
long ThorMCM3000Aux::SetParamString(const long paramID, wchar_t* str)
{
	long ret = FALSE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorMCM3000Aux::GetParamString(const long paramID, wchar_t* str, long size)
{
	long ret = FALSE;
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::PreflightPosition()
{
	return TRUE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::SetupPosition()
{	
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::StartPosition()
{
	long ret = FALSE;
...
/// <returns>long.</returns>
long ThorMCM3000Aux::StatusPosition(long &status)
{
	long ret = TRUE;
...
/// <returns>long.</returns>
long ThorMCM3000Aux::ReadPosition(DeviceType deviceType,double &pos)
{
	return FALSE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::PostflightPosition()
{
	return TRUE;
}
...
/// <returns>long.</returns>
long ThorMCM3000Aux::ExecuteCmd(ParamInfo* pParamInfo)
{
...
/// <returns>long.</returns>
long ThorMCM3000Aux::ExecuteCmd(std::vector<unsigned char> cmd, double &readBackValue)
{
	Lock lock(critSect);
...
/// </summary>
void ThorMCM3000Aux::ExecutePositionNow()
{
	PreflightPosition();
	SetupPosition();
	StartPosition();
	...
	do
	{
		...
		{
			StringCbPrintfW(message,MSG_SIZE,L"ThorMCM3000Aux::statusPosition failed");
			LogMessage(message);
			break;
		}
...
/// <returns>long.</returns>
long ThorMCM3000Aux::GetLastErrorMsg(wchar_t *msg, long size)
{
	return TRUE;
}
...
double ThorMCM3000Aux::rndup(double val,int decPlace)
{
...
std::vector<unsigned char> ThorMCM3000Aux::GetBytes(int value)
{
...
/// <returns>double.</returns>
double ThorMCM3000Aux::Round(double number, int decimals)
{
...
/// <returns>long.</returns>
long ThorMCM3000Aux::ResetStageZeroPosition(long stageID)
{
	int ret = TRUE;
	double currentPos;
	switch(stageID)
...
//long ThorMCM3000Aux::CheckMotorConnection()
//{
...
/// </summary>
/// <returns>long.</returns>
long ThorMCM3000Aux::BuildParamTable()
{
	_tableParams.clear();
...
void ThorMCM3000Aux::InvertMinMaxRange(double& min, double& max)
{
	double temp = min;
	min = -1 * max;
	max = -1 * temp;
