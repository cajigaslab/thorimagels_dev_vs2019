...
MCM6000Stage* MCM6000Stage::_instance = NULL;
	...
	)
{
	...
	case PARAM_DEVICE_TYPE:
		{
			...
			paramAvailable = TRUE;
			paramReadOnly = TRUE;
			paramMin = paramMax = paramDefault = (STAGE_X | STAGE_Y | STAGE_Z | STAGE_Z2| STAGE_R | LIGHT_PATH | FILTER_WHEEL_DIC);
		}
		break;
	...
	{
		paramType = TYPE_DOUBLE;
		paramAvailable = _device->_mcm6kParams->zeAvailable;
		paramReadOnly = FALSE;
		paramMin = -FLT_MAX;
		paramMax = FLT_MAX;
		paramDefault = 0;
...
long MCM6000Stage::GetParam(const long paramID, double &param)
{
	...
	case PARAM_R_POS_CURRENT:
		...
		break;
	case PARAM_DEVICE_TYPE:
		param = static_cast<double>(STAGE_X | STAGE_Y | STAGE_Z | STAGE_Z2| STAGE_R | LIGHT_PATH | FILTER_WHEEL_DIC);
		break;
	case PARAM_CONNECTION_STATUS:
		{
