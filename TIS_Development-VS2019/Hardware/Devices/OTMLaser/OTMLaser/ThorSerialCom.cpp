...
CThorSerialCom::CThorSerialCom()
{
	...
	_dfFreq = (double)_litmp.QuadPart;
}
...
BOOL CThorSerialCom::Open(LPWSTR PortName)
{
	...
	_hSerialPort = CreateFile(SerialPort,						// Specify port device
	...
	else
	{
		...
		SerialPortConfig.fRtsControl = 0;//HandShake;
		...
		if (SetCommState(_hSerialPort, &SerialPortConfig) != NULL)
		{
			...
			GetCommState(_hSerialPort, &SerialPortConfig);

			if (SetCommTimeouts(_hSerialPort, &commTimeout))
			{
				IsOpen = TRUE;
			}
			else
...
void CThorSerialCom::Close()
{
	if (_hSerialPort)
	{
		if(CloseHandle(_hSerialPort) == NULL)
		{
			IsOpen = TRUE;
		}
		else
...
void CThorSerialCom::SetComTimeout()
{
	if (_hSerialPort)
	{
		...
		SetCommTimeouts(_hSerialPort, &commTimeout);
	}
}
...
void CThorSerialCom::GetNumPorts(int *NumPorts)
{
	...
	*NumPorts = 0;
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"OTMLaser ThorSerialCom GetNumPorts error: %S",strCatchErr);
#ifdef LOGGING_ENABLED
		logDll->TLTraceEvent(VERBOSE_EVENT, 1, errmessage);
#endif
	}
...
void CThorSerialCom::GetPortInfo(int PortIndex, LPTSTR PortName)
{
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"OTMLaser ThorSerialCom GetPortInfo error: %S",strCatchErr);
#ifdef LOGGING_ENABLED
		logDll->TLTraceEvent(VERBOSE_EVENT, 1, errmessage);
#endif
	}
...
BOOL CThorSerialCom::Write(LPTSTR TxData)
{
	...
	//}
		...
		while(*pCurrent)
		{
			WriteFile(_hSerialPort,				// handle to file to write to
...
int CThorSerialCom::Read(LPTSTR RxData, int Timeout)
{
	...
	//modified
		...
		unsigned char cmd1[6] = { 0x6A, 0x04, 0x00, 0x00, 0x11, HOST_ID  };
	unsigned char cmd[]={0x16, 0x04, 22, 0, 0x11, HOST_ID,
                0, 0,  // Chan Ident
                2,0,                    // Jog Mode - single step jogging
                0x19, 0x01, 0x00, 0x00,    // Jog Step Size in encoder counts
	...
	if (_hSerialPort)
	{
		...
		_QPart1 = _litmp.QuadPart;

		while (!bGetPromptChar && !bTimeout)
		{
		
			_readFileSuccess=ReadFile(_hSerialPort,
					 RxBuffer,
					 BytesRead,
					 &BytesRead,
					 NULL);
			...
			QueryPerformanceCounter(&_litmp);
			_QPart2 = _litmp.QuadPart;
			_dfMinus = (double)(_QPart2 - _QPart1);
			_dfTim = _dfMinus/_dfFreq*1000;
...
{
	if (_hSerialPort)
		PurgeComm(_hSerialPort, PURGE_TXCLEAR |PURGE_RXCLEAR);
}
...
LPTSTR CThorSerialCom::parseSetting(LPTSTR commandToSend, LPTSTR settingToRead)
{
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"OTMLaser ThorSerialCom parseSetting error: %S",Str);
#ifdef LOGGING_ENABLED
		logDll->TLTraceEvent(VERBOSE_EVENT, 1, errmessage);
#endif
		return NULL;
...
void CThorSerialCom::usbWrite(byte *bytesToSend, int length)
{
...
BOOL CThorSerialCom::WriteCommByteArray(const unsigned char * byteArray, int size )
{
	BOOL bWriteStat;
	...
	if (size==6)
		bWriteStat = WriteFile(_hSerialPort, (LPSTR) byteArray, size, &dwBytesWritten, NULL );
	else
		bWriteStat = WriteFile(_hSerialPort, (LPSTR) byteArray, size, &dwBytesWritten, NULL );
