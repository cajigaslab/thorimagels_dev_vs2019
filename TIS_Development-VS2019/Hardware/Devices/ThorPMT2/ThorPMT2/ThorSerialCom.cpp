...
void CThorSerialCom::GetNumPorts(int *NumPorts)
{
	...
	*NumPorts = 0;
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"ThorPMT2 ThorSerialCom GetNumPorts error: %S",strCatchErr);
#ifdef LOGGING_ENABLED
...
void CThorSerialCom::GetPortInfo(int PortIndex, LPTSTR PortName)
{
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"ThorPMT2 ThorSerialCom GetPortInfo error: %S",strCatchErr);
#ifdef LOGGING_ENABLED
		logDll->TLTraceEvent(VERBOSE_EVENT, 1, errmessage);
#endif
	}
...
LPTSTR CThorSerialCom::parseSetting(LPTSTR commandToSend, LPTSTR settingToRead)
{
	...
	{
		StringCbPrintfW(errmessage,MSG_SIZE,L"ThorPMT2 ThorSerialCom parseSetting error: %S",Str);
#ifdef LOGGING_ENABLED
		logDll->TLTraceEvent(VERBOSE_EVENT, 1, errmessage);
#endif
		return NULL;
