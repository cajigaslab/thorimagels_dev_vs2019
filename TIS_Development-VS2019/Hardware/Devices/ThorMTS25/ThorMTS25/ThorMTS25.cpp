// ThorMTS25.cpp : Defines the exported functions for the DLL application.
//
...
/// <summary>
/// Prevents a default instance of the <see cref="ThorMTS25"/> class from being created.
/// </summary>
ThorMTS25::ThorMTS25()
{
	_numOfAxes = 1;
...
/// <summary>
/// Finalizes an instance of the <see cref="ThorMTS25"/> class.
/// </summary>
ThorMTS25::~ThorMTS25()
{
...
bool ThorMTS25::_instanceFlag = false;

auto_ptr<ThorMTS25> ThorMTS25::_single (new ThorMTS25());

const long ThorMTS25::MAX_AXIS_COUNT = 3;

/// <summary>
/// Gets the instance.
/// </summary>
/// <returns>ThorMTS25 *.</returns>
ThorMTS25* ThorMTS25::getInstance()
{
	if(!_instanceFlag)
	{
		_single.reset(new ThorMTS25());
		_instanceFlag = true;
		return _single.get();
	}
	else
...
long ThorMTS25::FindDevices(long &deviceCount)
{
	long ret = TRUE;
	...
	try
	{
		//Get portID, etc from hardware ThorMTS25Settings.xml
		auto_ptr<ThorMTS25XML> pSetup(new ThorMTS25XML());

		pSetup->GetConnection(portID,baudRate,address);
	}
	catch(...)
	{
		StringCbPrintfW(_errMsg,MSG_SIZE,L"Unable to locate ThorMTS25Settings.xml file");
		return FALSE;
	}

	if(FALSE == _serialPort.Open(portID, baudRate))
	{
		StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25 FindDevices could not open serial port");
		LogMessage(message);
		StringCbPrintfW(_errMsg,MSG_SIZE,L"ThorMTS25 FindDevices could not open serial port or configuration file is not avaible.");
...
/// <returns>long.</returns>
long ThorMTS25::SelectDevice(const long device)
{
	long ret = FALSE;
	...
	switch(device)
	{
		case 0:
		{
			...
			{				
				...
				//Get portID, etc from  ThorMTS25Settings.xml
				pSetup->GetConnection(portID,baudRate,address);
				
				//get the min and max and threshold for the XStage, YStage, ZStage from ThorMTS25Settings.xml
				pSetup->GetZRangeConfig(_zMin,_zMax, _zThreshold,zInvert,zHome);

				//get the stage type for each motorAxis from ThorMTS25Settings.xml
			...
			if(FALSE == _serialPort.Open(portID, baudRate))
			{
				StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25 SelectDevice could not open serial port");
				LogMessage(message);
				StringCbPrintfW(_errMsg,MSG_SIZE,L"ThorMTS25 SelectDevice could not open serial port or configuration file is not avaible.");
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::SetMotorParameters()
{
...
void ThorMTS25::LogMessage(wchar_t *message)
{
	#ifdef LOGGING_ENABLED
...
/// <returns>long.</returns>
long ThorMTS25::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
{
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::GetDeviceParameters()
{
	GetMotorType();
...
#pragma warning(pop)
		...
		if (0x3C == (unsigned char)buf[0] && 0x04 == (unsigned char)buf[1])
		{
			...
			_motorsParams[0].MotorEncCoef =  MOTOR_ENCODER_COEFFICIENT;
		}
	}
	return TRUE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::QueryInitialMotorPos()
{
	Lock lock(critSect);
	for(int i=0; i < _numOfAxes; i++)
	{
...
/// <returns>long.</returns>
long ThorMTS25::SetParam(const long paramID, const double param)
{
	double value = param;
	if (NULL != _tableParams[paramID])
	{
		if(_tableParams[paramID]->GetParamID() == paramID)
		{
			...
			{
				StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25 SetParam failed. paramID: %d", paramID);
				LogMessage(message);
				return FALSE;
			}
		}
...
/// <param name="param">The parameter.</param>
/// <returns>long.</returns>
long ThorMTS25::GetParam(const long paramID, double &param)
{
	double ret = -1;
	if (NULL != _tableParams[paramID])
	{
...
/// <returns>long.</returns>
long ThorMTS25::SetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorMTS25::GetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <returns>long.</returns>
long ThorMTS25::SetParamString(const long paramID, wchar_t* str)
{
	long ret = FALSE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorMTS25::GetParamString(const long paramID, wchar_t* str, long size)
{
	long ret = FALSE;
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::PreflightPosition()
{
	return TRUE;
}
...
/// <returns>long.</returns>
long ThorMTS25::StatusPosition(long &status)
{
	long ret = TRUE;
...
/// <returns>long.</returns>
long ThorMTS25::ReadPosition(DeviceType deviceType,double &pos)
{
	return FALSE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::PostflightPosition()
{
	return TRUE;
}
...
/// <returns>long.</returns>
long ThorMTS25::ExecuteCmd(ParamInfo* pParamInfo)
{
...
/// <returns>long.</returns>
long ThorMTS25::ExecuteCmd(std::vector<unsigned char> cmd, double &readBackValue)
{
	Lock lock(critSect);
...
/// </summary>
void ThorMTS25::ExecutePositionNow()
{
	PreflightPosition();
	SetupPosition();
	StartPosition();
	...
	do
	{
		...
		{
			StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25::statusPosition failed");
			LogMessage(message);
			break;
		}
...
/// <returns>long.</returns>
long ThorMTS25::GetLastErrorMsg(wchar_t *msg, long size)
{
	return TRUE;
}
...
double ThorMTS25::rndup(double val,int decPlace)
{
...
std::vector<unsigned char> ThorMTS25::GetBytes(int value)
{
...
/// <returns>double.</returns>
double ThorMTS25::Round(double number, int decimals)
{
...
/// <returns>long.</returns>
long ThorMTS25::ResetStageZeroPosition(long stageID)
{
	int ret = TRUE;
	double currentPos;
	switch(stageID)
	{
		...
		{			
			...
			{
				StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25::ResetStageZeroPosition GetParam of PARAM_Z_POS_CURRENT failed");
				LogMessage(message);
			}
			...
			if ((currentPos < _zMin) || (currentPos > _zMax))
			{
				...
				{
					StringCbPrintfW(message,MSG_SIZE,L"ThorMTS25::ResetStageZeroPosition SetParam of PARAM_Z_ZERO failed");
					LogMessage(message);
				}
				ExecutePositionNow();
			}
...
//long ThorMTS25::CheckMotorConnection()
//{
...
/// </summary>
/// <returns>long.</returns>
long ThorMTS25::BuildParamTable()
{
	_tableParams.clear();
	...
	tempParamInfo = new ParamInfo(	
					...
					(double)ConnectionStatusType::CONNECTION_READY,	//VAL
					(double)ConnectionStatusType::CONNECTION_READY,	//PARAM C
					FALSE,									//PARAM B
					TYPE_LONG,								//TYPE
					TRUE,									//AVAILABLE
					TRUE,									//READ ONLY
					...
					(double)ConnectionStatusType::CONNECTION_READY,	//DEFAULT
					-1,										//Motor ID
					commandBytes);							//Command
...
void ThorMTS25::InvertMinMaxRange(double& min, double& max)
{
	double temp = min;
	min = -1 * max;
	max = -1 * temp;
