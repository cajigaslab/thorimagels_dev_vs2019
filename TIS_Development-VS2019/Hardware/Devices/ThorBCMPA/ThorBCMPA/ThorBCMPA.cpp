...
/// </summary>
wchar_t message[256];
const string ThorBCMPA::_deviceSignature[DEVICE_NUM] = {"PA1"};
...
/// </summary>
ThorBCMPA::ThorBCMPA():WAIT_TIME_BETWEEN_SEND_COMMANDS(5)
{
	_errMsg[0] = NULL;
...
bool ThorBCMPA::_instanceFlag = false;
auto_ptr<ThorBCMPA> ThorBCMPA::_single (new ThorBCMPA());
CritSect ThorBCMPA::critSec;

/// <summary>
/// Gets the instance.
/// </summary>
/// <returns>ThorBCMPA *.</returns>
ThorBCMPA* ThorBCMPA::getInstance()
{
	Lock lock(critSec);

	if(!_instanceFlag)
	{
		_single.reset(new ThorBCMPA());
		_instanceFlag = true;
	}
	return _single.get();
}
...
long ThorBCMPA::FindDevices(long &deviceCount)
{
	Lock lock(critSec);
	...
	if (serialNum[0].size() > 0)
	{
		...
		if (TRUE == _serialPort[0].Open(sNoPortID, 460800))
		{
			...
			_serialPort[0].Close();
			_foundByPID = TRUE;
			/*Still need _useShutter tag*/
			auto_ptr<ThorBCMPAXML> pSetup(new ThorBCMPAXML());
...
/// <returns>long.</returns>
long ThorBCMPA::SelectDevice(const long device)
{
	Lock lock(critSec);
	long ret = FALSE;
	long portID=0;
...
/// <returns>long.</returns>
long ThorBCMPA::VerifySerialNumbers(long portId[])
{
	long ret = TRUE;
	if (TRUE == _deviceDetected[DEVICE_NUM])
	{
...
/// <returns>long.</returns>
long ThorBCMPA::GetDeviceSerialNumber(long deviceIndx, string &deviceSerialNumber)
{
...
void ThorBCMPA::LogMessage(wchar_t *message)
{
#ifdef LOGGING_ENABLED
...
/// <returns>long.</returns>
long ThorBCMPA::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
{
...
/// <returns>long.</returns>
long ThorBCMPA::SetParam(const long paramID, const double param)
{
	if (NULL != _tableParams[paramID])
	{
		if(_tableParams[paramID]->GetParamID() == paramID)
...
double ThorBCMPA::Round(double number, int decimals)
{
...
/// <param name="param">The parameter.</param>
/// <returns>long.</returns>
long ThorBCMPA::GetParam(const long paramID, double &param)
{
	double ret = -1;
	if (NULL != _tableParams[paramID])
	{
...
/// <returns>long.</returns>
long ThorBCMPA::SetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorBCMPA::GetParamBuffer(const long paramID, char* pBuffer, long size)
{
	long ret = TRUE;
...
/// <returns>long.</returns>
long ThorBCMPA::SetParamString(const long paramID, wchar_t* str)
{
	long ret = FALSE;
...
/// <param name="size">Buffer size.</param>
/// <returns>long.</returns>
long ThorBCMPA::GetParamString(const long paramID, wchar_t* str, long size)
{
	long ret = FALSE;
...
/// <returns>long.</returns>
long ThorBCMPA::ExecuteCmd(ParamInfo* pParamInfo)
{
...
/// Now use StatusPosition
/// </summary>
/// <param name="deviceIndex">Index of the device.</param>
/// <returns>long.</returns>
//long ThorBCMPA::WaitForRotationComplete(long deviceIndex)
//{
//	char moving = 0;
//	char buf[100];
...
/// <returns>long.</returns>
long ThorBCMPA::ExecuteCmd(long paramID, std::vector<unsigned char> cmd, double &readBackValue)
{
	Lock lock(critSec);
...
/// </summary>
/// <returns>long.</returns>
long ThorBCMPA::BuildParamTable()
{
	const double MAX_POWER_INPUT_POSITION = 100.0;
	_tableParams.clear();
	...
	//retrieve the current position of the power device
	unsigned char commandBytesBCMPA0Encoder[] = { 0x0A, 0x04, 0x00, 0x00, 0x00, 0x00 }; 
	commandBytes.assign(commandBytesBCMPA0Encoder, commandBytesBCMPA0Encoder + sizeof(commandBytesBCMPA0Encoder)/sizeof(commandBytesBCMPA0Encoder[0]));
	tempParamInfo = new ParamInfo(
		PARAM_POWER_ENCODER_POS,	//ID
		0,					//VAL
		0,					//PARAM C
		TRUE,				//PARAM B
		TYPE_DOUBLE,		//TYPE
		...
		NULL,				//&LinearizeSine,
		0,					//MIN
		TOTAL_COUNTS_PER_MOTOR_REVOLUTION,	//MAX
		0,					//DEFAULT
		1,					//Motor ID
		commandBytes);		//COMMAND
	_tableParams.insert(std::pair<long, ParamInfo*>(PARAM_POWER_ENCODER_POS,tempParamInfo));
...
/// <returns>long.</returns>
long ThorBCMPA::GetLastErrorMsg(wchar_t *msg, long size)
{
	return TRUE;
}
...
/// <returns>long.</returns>
long ThorBCMPA::StatusPosition(long &status)
{
	char moving = 0;
	char buf[100];
...
/// <returns>long.</returns>
long ThorBCMPA::ReadPosition(DeviceType deviceType,double &pos)
{
	return FALSE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorBCMPA::PostflightPosition()
{
	return TRUE;
}
...
/// </summary>
/// <returns>long.</returns>
long ThorBCMPA::PreflightPosition()
{
	return TRUE;
}
