...
extern "C"
{
	...
	private:
		...
		const unsigned int UNIT_TRANSIENT_FRAME_US;
		...
		unsigned int _unitTimeoutMS; ///<timeout setting [MS] to write a single frame
	...
	private:
		MeadowlarkPDM();
		BOOL IsOverdrive() { return ((_overDrive) && (0 == _pSlmName.compare(EnumString<SLMHardware>::From(SLMHardware::PDM1024)))) ? TRUE : FALSE; }
		void ReleaseMem();
