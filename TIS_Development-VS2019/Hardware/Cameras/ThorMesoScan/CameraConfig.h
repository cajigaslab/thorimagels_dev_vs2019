...
public: 
	...
	//********** NI card setting **********
	//channel strings
	char P1_GY_GX1_P2_AO_CHANNEL[64];
	char POCKEL_AO_CHANNEL[64];
	char GX2_AO_CHANNEL[64];
	char VOICECOIL_AO_CHANNEL[64];
	char GY_VOICECOIL_FEEDBACK_CHANNEL[64];
	char GX1_GX2_FEEDBACK_CHANNEL[64];
	char RESONANT_AO_CHANNEL[64];
	char RESONANT_CTR[64];
	char FRAMETRIGGEROUT_DO_CHANNEL[64];
	char TRIGGER_IN_CHANNEL[64];
	char PDA_CHANNEL[64];
	//sample rate
	long DEFAULT_EXTCLOCKRATE;
	...
	//********** Alazar board parameters **********
	bool IsDataSkippingAveraging;	//data skipping with pixel averaging
	unsigned int RecordPerBuffer;	//number of records in each buffer
	unsigned int TriggerLevel;		//voltage level that the trigger source signal for the specified trigger engine must pass through to generate a trigger event
	double InputRange;
	double MaxPMTVoltage;
	double MinPMTVoltage;
	double ResonantAmplitude;
	...
	unsigned int DmaCount;									//dma buffer count
	...
	unsigned int LsmChannel;								//lsm channels, [A]:1, [B]:2, [C]:3, [D]:4, [all]:15
	...
	unsigned int TriggerMode;								//trigger mode
	unsigned int RawSaveEnabledChannelOnly;					//save enabled channel only if true
	unsigned int DroppedFramesCount;						//number of frame missed by copy acquisition
	...
	double TriggerWaitTimeSec;								//timeout in seconds for hardware trigger
private:
