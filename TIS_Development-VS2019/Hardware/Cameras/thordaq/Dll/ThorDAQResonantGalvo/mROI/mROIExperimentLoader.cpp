...
long mROIExperimentLoader::LoadExperimentXML()
{
	if (0 >= ExpPath.size())
		return 0;
	...
	try
	{
		...
		for (tinyxml2::XMLElement* child = templateScans->FirstChildElement("ScanInfo"); child != NULL; child = child->NextSiblingElement("ScanInfo"))
		{
			...
			tScan->MaxSizeInUMForMaxFieldSize = child->DoubleAttribute("MaxSizeInUMForMaxFieldSize");
			tScan->IsmROI = 0 != scanType.compare("Meso");

			if (0 != scanType.compare("Meso"))
			{
				tScan->FullFOVPhysicalSizeUM = physicalFieldSizeUM;
			}
			...
			if (NULL != scanConfigure)
			{
				...
				tScan->ScanConfig.StripeFieldSize = scanConfigure->IntAttribute("StripeFieldSize");
			}
			...
			if (NULL != scanAreas)
			{
				for (tinyxml2::XMLElement* child1 = scanAreas->FirstChildElement("ScanArea"); child1 != NULL; child1 = child1->NextSiblingElement("ScanArea"))
				{
					if (0 == scanType.compare("Meso"))
					{
						physicalFieldSizeUM = child1->DoubleAttribute("PhysicalSizeX");
					}
					...
					if (NULL != powerPoints)
					{
						for (tinyxml2::XMLElement* child2 = powerPoints->FirstChildElement("PowerPoint"); child2 != NULL; child2 = child2->NextSiblingElement("PowerPoint"))
						{
							...
							sArea->PowerPoints0.push_back(pPoint);
						}
					}
