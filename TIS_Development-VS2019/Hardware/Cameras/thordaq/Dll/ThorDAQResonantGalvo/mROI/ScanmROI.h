...
public:
	ScanArea() {}
	~ScanArea() {
		if (0 < PowerPoints0.size())
		{
			for (size_t i = 0; i < PowerPoints0.size(); i++)
			{
				delete PowerPoints0.at(i);
			}
			PowerPoints0.clear();
		}
		if (0 < PowerBoxs.size())
	...
	ScanArea(const ScanArea& source)
	{
		...
		vector<PowerPoint*> pps = source.PowerPoints0;
		for (vector<PowerPoint*>::iterator iter = pps.begin(); iter != pps.end(); iter++)
		{
			PowerPoints0.push_back(new PowerPoint(**iter));
		}
	...
	double PositionY;
	double PositionZ;
	vector<PowerPoint*> PowerPoints0;
	vector<PowerPoint*> PowerPoints1;
	vector<PowerPoint*> PowerPoints2;
	vector<PowerPoint*> PowerPoints3;
...
public:
	...
	Scan(const Scan& source)
	{
		...
		IsmROI = source.IsmROI;
		Name = source.Name;
		FullFOVPhysicalSizeUM = source.FullFOVPhysicalSizeUM;
		MaxSizeInUMForMaxFieldSize = source.MaxSizeInUMForMaxFieldSize;
