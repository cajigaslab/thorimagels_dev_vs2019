...
// Externally Visible API
	...
	// Now that we have multiple boards, e.g. ThorDAQ 0x4001, dFLIM 0x4002, etc.
	// provide API that returns device code of detected PCIe board we cannot
	// connect to (e.g. ThorDAQ DLL cannot connect to dFLIM board, etc.)
	THORDAQ_API THORDAQ_STATUS ThorDAQAPIAttemptConnection
	(
		UINT				boardNum, UINT32* PCIBARdevID                  // Board to target
	);
...
//	THORDAQ_API THORDAQ_STATUS ThorDAQAPItdCancelUserIntTask(UINT32 board);
	...
	// 
// This API blocks forever waiting for either:
// A. Completion of IOCTL with "success" by kernel driver FPGA User Int reception
// B. Call by ThorDAQAPICancelWaitForUserIRQ() which completes IOCTL with "timeout"
// Call returns count of User Ints per channel (from FPGA status regs - typically all the same)
