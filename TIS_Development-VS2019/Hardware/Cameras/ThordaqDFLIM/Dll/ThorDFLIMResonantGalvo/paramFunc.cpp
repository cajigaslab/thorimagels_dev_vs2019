...
long CThordaqResonantGalvo::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
{
	...
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_FINE_251:
		{
			...
			paramAvailable = TRUE;
			paramMin = 0;
			paramMax = (double)SYS_CLOCK_FREQ /_crsFrequencyHighPrecision/2.0;
			paramDefault = 0;
			paramReadOnly = FALSE;
		}
		break;
	...
	case ICamera::PARAM_LSM_POCKELS_MASK_WIDTH:
		{
			paramType = ICamera::TYPE_LONG;
			paramAvailable = TRUE;
			paramMin = _pockelsMaskWidth;
			paramMax = _pockelsMaskWidth;
			paramDefault = _pockelsMaskWidth;
			paramReadOnly = TRUE;
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_OUTPUT_USE_REF:
		{
			paramType = ICamera::TYPE_LONG;
			paramAvailable = FALSE; //Only available when using an NI PCIe-6363 Dev card
			paramMin = FALSE;
			paramMax = TRUE;
			paramDefault = FALSE;
			paramReadOnly = FALSE;
...
long CThordaqResonantGalvo::SetParam(const long paramID, const double param)
{
	...
	case ICamera::PARAM_LSM_CLOCKSOURCE:
		{
			if (_imgAcqPty.clockSource != static_cast<long> (param))
			{
				...
				if (_deviceNum != 0)
				{
					...
					{
						//ThordaqSetClockSource(_DAQDeviceIndex, CLOCK_SOURCE::INTERNAL_80MHZ_REF, DEFAULT_INTERNALCLOCKRATE);
					}else
					{
						//ThordaqSetClockSource(_DAQDeviceIndex, CLOCK_SOURCE::EXTERNAL_CLOCK_SOURCE, 2 * _imgAcqPty.clockRateExternal);
					}
	...
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_FINE_251:
		{
			if((param >= 0) && (param < (double)SYS_CLOCK_FREQ /_crsFrequencyHighPrecision/2.0 ))
			{
				_imgAcqPty.twoWayZonesFine[paramID - ICamera::PARAM_LSM_TWO_WAY_ZONE_FINE_1] = static_cast<long>(param);
				ret = TRUE;
			}
			else
			{
	...
	case ICamera::PARAM_LSM_POCKELS_OUTPUT_USE_REF:
		{
			...
			{
				_useReferenceForPockelsOutput = static_cast<long>(param);
			}
		}
		break;
...
long CThordaqResonantGalvo::GetParam(const long paramID, double &param)
{
	...
	case ICamera::PARAM_LSM_INPUTRANGE1:
		{
			param  = _imgAcqPty.inputRangeChannel1;
		}
		break;
	case ICamera::PARAM_LSM_INPUTRANGE2:
		{
			param  = _imgAcqPty.inputRangeChannel2;
		}
		break;
	case ICamera::PARAM_LSM_INPUTRANGE3:
		{
			param  = _imgAcqPty.inputRangeChannel3;
		}
		break;
	case ICamera::PARAM_LSM_INPUTRANGE4:
		{
			param  = _imgAcqPty.inputRangeChannel4;
		}
		break;
	case ICamera::PARAM_LSM_CLOCKSOURCE:
		{
	...
	case ICamera::PARAM_LSM_POCKELS_MASK_ENABLE_0:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskEnable[0];
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_MASK_ENABLE_1:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskEnable[1];
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_MASK_ENABLE_2:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskEnable[2];
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_MASK_INVERT_0:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskInvert[0];
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_MASK_INVERT_1:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskInvert[1];
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_MASK_INVERT_2:
		{
			param = _imgAcqPty.pockelPty.pockelsMaskInvert[2];
		}
		break;
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_1:
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_2:
	...
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_FINE_251:
		{
			param = _imgAcqPty.twoWayZonesFine[paramID - ICamera::PARAM_LSM_TWO_WAY_ZONE_FINE_1];
		}
		break;
	case ICamera::PARAM_LSM_FORCE_SETTINGS_UPDATE:
		{
	...
	case ICamera::PARAM_LSM_POCKELS_MASK_WIDTH:
		{
			param = (double)_pockelsMaskWidth;
		}
		break;
	case ICamera::PARAM_LSM_POCKELS_OUTPUT_USE_REF:
		{
