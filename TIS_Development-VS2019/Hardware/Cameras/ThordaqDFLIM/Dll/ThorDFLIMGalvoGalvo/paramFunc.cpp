...
long CThorDAQGalvoGalvo::SetParam(const long paramID, const double param)
{
	...
	case ICamera::PARAM_LSM_FIELD_SIZE:
		{
			if ((param >= MIN_FIELD_SIZE_X) && (param <= MAX_FIELD_SIZE_X))
			{
				...
				long minFieldSize = CalculateMinimumFieldSize(_imgAcqPty.dwellTime, _imgAcqPty.pixelX, _imgAcqPty.turnAroundTimeUS, _field2Theta, MAX_GALVO_OPTICAL_ANGLE);
				if(param > minFieldSize && FALSE == _ggSuperUserMode)
				{
					...
					StringCbPrintfW(_errMsg,MSG_SIZE,L"PARAM_LSM_FIELD_SIZE results in a voltage that is out of range. Moving to minimum safe value.");
					LogMessage(_errMsg,ERROR_EVENT);
				}

				double paddedAmplitude = fieldSizeParam * GALVO_RETRACE_TIME * 4 / ( _imgAcqPty.pixelX * _imgAcqPty.dwellTime * 2 * M_PI);
				double fieldX_angle = (fieldSizeParam + paddedAmplitude *2) * _field2Theta;

				double paddedAmplitudeY = fieldSizeParam * GALVO_RETRACE_TIME * 4 / ( _imgAcqPty.pixelY * _imgAcqPty.dwellTime * 2 * M_PI);
				...
				{
					_imgAcqPty.fieldSize = static_cast<long>(fieldSizeParam);
					ret = TRUE;
				}
			}
			else
	...
	case ICamera::PARAM_LSM_3P_ALIGN_COURSE:
		{
			_imgAcqPty.threePhotonModeAlignmentPhase = static_cast<long>(param);
		}
		break;
	case ICamera::PARAM_LSM_GG_SUPER_USER:
		{
