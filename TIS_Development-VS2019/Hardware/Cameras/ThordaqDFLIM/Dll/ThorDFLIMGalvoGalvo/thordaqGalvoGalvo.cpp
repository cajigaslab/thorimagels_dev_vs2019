					...
					for (int i = 0; i < MAX_CHANNEL_COUNT; i++)
					{
						if ((_daqAcqCfg.imageCtrl.channel & (0x0001 << i)))
						{
							size = static_cast<ULONG>(frameSizeByte * frame_left); 
																												// ACQ_SINGLE_CHANNEL_BUF_CAP, maximum single chan DMA frame len (120MB)
							ULONG64 CardOffset = i * DDR3_SINGLEIMAGE_CHANNEL_CAP + DDR3_IMAGE_2ndPINGPONGBUFFstart * bank;	// ACQ_MSI_BUF_OFFSET is DDR3 address of start of 2nd ping-pong buffer
...
void CThorDAQGalvoGalvo::StopDaqBrd()
{
	...
	ThordaqStopAcquisition(_DAQDeviceIndex); // makes kernel call IMG_ACQ_CONF_IOCTL  
}
