...
const char * const ThorGalvoGalvoXML::CONFIG_ATTR[NUM_CONFIG_ATTRIBUTES] = {"field2Theta","pockelsParkAtMinimum"};

long ThorGalvoGalvoXML::GetConfiguration(double &field2Theta, long &pockelsParkAtMinimum)
{	
	StringCbPrintfW(_currentPathAndFile,MAX_PATH,L"ThorDAQGGDFLIMSimSettings.xml");		
	...
	else
	{
		...
		StringCbPrintfW(errMsg,MSG_SIZE,L"ThorDAQGGDFLIMSimSettings GetConfiguration failed");
		CThorDAQGGDFLIMSim::GetInstance()->LogMessage(errMsg,VERBOSE_EVENT);
		}
	}	
	return TRUE;
}
...
{	
	...
	else
	{
		...
		{
			wchar_t errMsg[MSG_SIZE];
			StringCbPrintfW(errMsg,MSG_SIZE,L"ThorDAQGGDFLIMSimSettings GetCalibration failed");
			CThorDAQGGDFLIMSim::GetInstance()->LogMessage(errMsg,VERBOSE_EVENT);
			return FALSE;
		}
	}
...
long ThorGalvoGalvoXML::GetStreamConfiguration(long &samplerate, long &FIRFilter1, long &FIRFilter2, double &DCOffset1, double &DCOffset2)
{
	...
	else
	{
		...
		{
			wchar_t errMsg[MSG_SIZE];
			StringCbPrintfW(errMsg,MSG_SIZE,L"ThorDAQGGDFLIMSimSettings GetStream failed");
			CThorDAQGGDFLIMSim::GetInstance()->LogMessage(errMsg,VERBOSE_EVENT);
			return FALSE;
		}
	}
