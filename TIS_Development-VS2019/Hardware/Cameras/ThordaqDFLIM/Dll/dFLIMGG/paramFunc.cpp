...
/**********************************************************************************************//**
 * @fn	long CdFLIMGG::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
 ...
 **************************************************************************************************/
long CdFLIMGG::GetParamInfo(const long paramID, long &paramType, long &paramAvailable, long &paramReadOnly, double &paramMin, double &paramMax, double &paramDefault)
{
	long ret = TRUE;
...
long CdFLIMGG::SetParam(const long paramID, const double param)
{
	long ret = FALSE;
	...
	case ICamera::PARAM_ENABLE_FRAME_TRIGGER_WITH_HW_TRIG:
		...
		// DZimmerman -- add "maxDwell" to constrain upper limit of DDR3 DMA len (no more than 224 MB) 
	case ICamera::PARAM_LSM_DWELL_TIME:
		{
		THORDAQ_STATUS status = THORDAQ_STATUS::STATUS_SUCCESSFUL;
		double computedMaxDwell = param; // need to test USER entered param for upper bound
		double maxDwell = MAX_DWELL_TIME; // in case config-computed function fails
			if ((param >= MIN_DWELL_TIME) && (param <= MAX_DWELL_TIME))
			{
				...
				// compute and if necessary adjust the Dwell for the DDR3 DMA image frame fits in the physically allocated space (e.g. 224 MB)
				status = ConstrainHighestPixelDwell(_imgAcqPty.pixelX, _imgAcqPty.pixelY, &computedMaxDwell);
				if (status == THORDAQ_STATUS::STATUS_SUCCESSFUL)
				{
					 maxDwell = computedMaxDwell; // dwell has been lowered from the passed param
				}
				// correct Dwell too short
				_imgAcqPty.dwellTime = (param > minDwell || TRUE == _ggSuperUserMode) ? param : minDwell;
				// correct Dwell too long
				if(_imgAcqPty.dwellTime > maxDwell)  _imgAcqPty.dwellTime = maxDwell;
				ret = TRUE;
			}
			else
			{
	...
	case ICamera::PARAM_DFLIM_RESYNC:
		{
			...
			ThordaqErrChk2 (L"ThorDAQAPIdFLIMReSync", status = ThorDAQAPIdFLIMReSync(_DAQDeviceIndex));
		}
		break;
	case ICamera::PARAM_DFLIM_COARSE_SHIFT1:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_COARSE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_COARSE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 0;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetCoarseShift", status = ThorDAQAPIdFLIMSetCoarseShift(_DAQDeviceIndex, (ULONG32)_dflimCoarseShiftA[0], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_COARSE_SHIFT2:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_COARSE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_COARSE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 1;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetCoarseShift", status = ThorDAQAPIdFLIMSetCoarseShift(_DAQDeviceIndex, (ULONG32)_dflimCoarseShiftA[1], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_COARSE_SHIFT3:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_COARSE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_COARSE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 2;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetCoarseShift", status = ThorDAQAPIdFLIMSetCoarseShift(_DAQDeviceIndex, (ULONG32)_dflimCoarseShiftA[2], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_COARSE_SHIFT4:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_COARSE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_COARSE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 3;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetCoarseShift", status = ThorDAQAPIdFLIMSetCoarseShift(_DAQDeviceIndex, (ULONG32)_dflimCoarseShiftA[3], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_FINE_SHIFT1:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_FINE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_FINE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 0;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetFineShift", status = ThorDAQAPIdFLIMSetFineShift(_DAQDeviceIndex, (LONG32)_dflimFineShiftA[0], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_FINE_SHIFT2:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_FINE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_FINE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 1;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetFineShift", status = ThorDAQAPIdFLIMSetFineShift(_DAQDeviceIndex, (LONG32)_dflimFineShiftA[1], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_FINE_SHIFT3:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_FINE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_FINE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 2;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetFineShift", status = ThorDAQAPIdFLIMSetFineShift(_DAQDeviceIndex, (LONG32)_dflimFineShiftA[2], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_FINE_SHIFT4:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_FINE_SHIFT && static_cast<long>(param) >= MIN_DFLIM_FINE_SHIFT)
			{
				...
				long error = 0, status = 0;
				int channel = 3;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetFineShift", status = ThorDAQAPIdFLIMSetFineShift(_DAQDeviceIndex, (LONG32)_dflimFineShiftA[3], channel));
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_DFLIM_QUERY_CLOCK_FREQS:
		{
			...
			{
				long error = 0, status = 0;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMGetClockFrequency", status = ThorDAQAPIdFLIMGetClockFrequency(_DAQDeviceIndex, i, _dflimClockFrequencies[i]));
			}
			ret = TRUE;
		}
		break;
	case ICamera::PARAM_DFLIM_SYNC_DELAY:
		{
			if (static_cast<long>(param) <= MAX_DFLIM_SYNC_DELAY && static_cast<long>(param) >= MIN_DFLIM_SYNC_DELAY)
			{
				...
				long error = 0, status = 0;
				ThordaqErrChk2 (L"ThorDAQAPIdFLIMSetSyncingSettings", status = ThorDAQAPIdFLIMSetSyncingSettings(_DAQDeviceIndex, static_cast<ULONG32>(_dflimSyncDelay), static_cast<ULONG32>(_dflimResyncDelay), TRUE == _dflimResyncEveryLine));
				ret = TRUE;
			}
		}
		break;
	...
	if ((!_imgAcqPty.IsEqual(_imgAcqPty_Pre)) && ICamera::SW_FREE_RUN_MODE == _imgAcqPty.triggerMode)
	{
...
/**********************************************************************************************//**
 * @fn	long CdFLIMGG::GetParam(const long paramID, double &param)
 *
 * @brief	Gets parameter information.
 *
 * @param	paramID				  	Identifier for the parameter.
 ...
 * @return	A long.
 **************************************************************************************************/
long CdFLIMGG::GetParam(const long paramID, double &param)
{
	long ret = TRUE;
