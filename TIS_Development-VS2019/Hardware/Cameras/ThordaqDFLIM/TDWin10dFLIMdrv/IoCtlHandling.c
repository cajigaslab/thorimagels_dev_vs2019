	...
	case PACKET_CHANNEL_READ_IOCTL:
		{
			...
			int                         i =0;
#ifdef Debug
			PBAR2_CONTROL_MAP_STRUCT pBar2Controls = pDevExt->BarVirtualAddress[2];
			//DEBUGP(DEBUG_INFO,	"Enter PACKET_CHANNEL_READ_IOCTL");
#endif
            status = STATUS_INVALID_PARAMETER;
			// Make sure the Input size is what we expect
			if (InputBufferLength >= sizeof(PACKET_READ_STRUCT))
									...
									(pDevExt->pDmaEngineDevExt[pReadPacket->EngineNum] != NULL))
								{
									...
									if (OutputBufferLength >= sizeof(PACKET_RET_READ_STRUCT)) //(size_t) (pReadPacket->Length))
									{
										...
										if (pDevExt->pDmaEngineDevExt[pReadPacket->EngineNum]->DmaType == DMA_TYPE_PACKET_READ)
										{
											if (pDevExt->pDmaEngineDevExt[pReadPacket->EngineNum]->PacketMode == PACKET_MODE_ADDRESSABLE)
											{
											//	DEBUGP(DEBUG_INFO, "  pReadPacket->Channel = %d ", pReadPacket->Channel);
												...
												if(pReadPacket->ModeFlags == READ_WRITE_MODE_FLAG_ADDRESSED)
												{
													if (pS2mmEngExt->IsDataReady == 1)
													{
														...
														pReadPacket->CardOffset = pDevExt->DataBufferStartAddress + ((ULONG64)pReadPacket->Channel * pDevExt->MAXframeLengthPerChan) + (ULONG64)pDevExt->pS2mmLayerExt->AcqBufOffset;
														DEBUGP(DEBUG_INFO, "@PACKET_CHANNEL_READ_IOCTL calling PacketStartRead, ->Chan %d, DDR3addr(CardOffset) 0x%x, BufferAddress 0x%x, Length %d (0x%x), end DDR3 0x%x",
															pReadPacket->Channel, (int)pReadPacket->CardOffset, (int)pReadPacket->BufferAddress, pReadPacket->Length, pReadPacket->Length,
															(int)(pReadPacket->CardOffset + pReadPacket->Length));
														status = PacketStartRead(Request, pDevExt, pReadPacket);
														DEBUGP(DEBUG_INFO, " -- PacketStartRead status 0x%x", status);
														...
														if (interruptClearFlag == TRUE)
														{
															for (i = 0; i < S2MM_CHANNEL_PER_BOARD; i++)
															{
																...
																{
																	DEBUGP(DEBUG_INFO, "pBar2Controls->ctrlCh[%d].ctrl.SR0_CR0 (ptr 0x%p) =>0x53 (Re-Arm|CyCD Start|Norm) ", i, &pBar2Controls->ctrlCh[i].ctrl.SR0_CR0);
												...
												else
												{
													...
													DEBUGP(DEBUG_INFO, "calling PacketStartRead, ->Chan %d, CardOS 0x%x, BufferAddress 0x%x, Length %d", 
														pReadPacket->Channel, (int)pReadPacket->CardOffset, (int)pReadPacket->BufferAddress, pReadPacket->Length);
													status = PacketStartRead(Request, pDevExt, pReadPacket);
	...
	case PACKET_WRITE_IOCTL:
		{
			...
			status = STATUS_INVALID_PARAMETER;
			DEBUGP(DEBUG_INFO,	"Enter PACKET_WRITE_IOCTL");
			// Make sure the Input size is what we expect
