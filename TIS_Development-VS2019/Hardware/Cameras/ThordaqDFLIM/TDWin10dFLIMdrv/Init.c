	...
	)
{
	...
	DEBUGP(DEBUG_INFO, "Enter ThorDaqDrvS2mmDmaInit: ");
	// Setup Bar 1
	barNum = 1;
		...
		(pDevExt->BarType[barNum] == CmResourceTypeMemoryLarge)))
	{
		...
		DEBUGP(DEBUG_INFO, "BAR%d pDevExt->pBar1Descriptors 0x%p ", barNum, pDevExt->pBar1Descriptors);
		...
		(pDevExt->BarType[barNum] == CmResourceTypeMemoryLarge)))
	{
		...
		DEBUGP(DEBUG_INFO, "BAR%d pDevExt->pBar2Controls 0x%p ", barNum, pDevExt->pBar2Controls);
		...
		(pDevExt->BarType[barNum] == CmResourceTypeMemoryLarge)))
	{
		...
		DEBUGP(DEBUG_INFO, "BAR%d pDevExt->pBar3Map 0x%p ", barNum, pDevExt->pBar3Map);
	...
	pBar3->globalGenCtrl.Layout.StopRun_FpgaRev = 0x08; // HW Triggering
	DEBUGP(DEBUG_INFO, "pDevExt->pBar3Map (ptr) 0x%p set to 0x08", &pDevExt->pBar3Map);
	DEBUGP(DEBUG_INFO, "pBar3->globalGenCtrl.Layout.StopRun_FpgaRev (ptr) 0x%p ", &pBar3->globalGenCtrl.Layout.StopRun_FpgaRev);
	...
	frameSize = hSize * vSize;  // 0x1000
	pDevExt->FrameSizeInPixels = inHSizeByPixels * vSize;
	frameIndexCap = (0x10000000 / S2MM_DESCRS_PER_BLK / (inHSizeByBytes*vSize));

	DEBUGP(DEBUG_INFO, "                             hSize %d, vSize %d, inHSizeByPixels %d, inHSizeByBytes %d, frameSize %d, frameIndexCap %d",
		hSize, vSize, inHSizeByPixels, inHSizeByBytes, frameSize, frameIndexCap);
	...
	DEBUGP(DEBUG_INFO, "pDevExt->DataChannelLength %d (0x%x), FrameRate %d", pDevExt->MAXframeLengthPerChan, pDevExt->MAXframeLengthPerChan, pDevExt->FrameRate);
	...
	{		
		...
		DEBUGP(DEBUG_INFO, "chan %d, pS2mmCtrl->SR0_CR0 0x%p  write 0x%x", ch, &pS2mmCtrl->SR0_CR0, 0x52);
		pS2mmCtrl->SR0_CR0 = 0x52;// 0x12;


		pS2mmCtrl->ChainStartAddr = 0x0;
		pS2mmCtrl->ChainTailAddr = 0x510; //an invalid address to stop S2mm DMA
		DEBUGP(DEBUG_INFO, "chan %d, pS2mmCtrl->ChainTailAddr 0x%p  write 0x510", ch, &pS2mmCtrl->ChainTailAddr);
		pS2mmCtrl->ChainIrqThreshold = (USHORT)(pDevExt->FrameRate); //0x1;
		DEBUGP(DEBUG_INFO, "chan %d, pS2mmCtrl->ChainIrqThreshold 0x%p  write 0x52", ch, &pS2mmCtrl->ChainIrqThreshold);
		for(blk = 0; blk < pDevExt->FrameRate; blk++)
		{
			for(descr = 0; descr < S2MM_DESCRS_PER_BLK; descr++)
			{
				...
				if( blk < 2) DEBUGP(DEBUG_INFO, "descr %d, pDescriptor 0x%p ", descr, pDescriptor);

				pDescriptor->NxtDescPtr = (((blk + 1) % pS2mmExt->IndexCap) * sizeof(S2MM_DESC_BLK_STRUCT) +  sizeof(S2MM_DMA_DESCRIPTOR_STRUCT) * descr +  ((ULONGLONG)ch<<16));
				if (blk < 2) DEBUGP(DEBUG_INFO, "descr %d, pDescriptor->NxtDescPtr 0x%p => 0x%x", descr, &pDescriptor->NxtDescPtr, pDescriptor->NxtDescPtr);

				pDescriptor->BuffAddr = pDevExt->DataBufferStartAddress + ((ch + 0) % 4) * pDevExt->MAXframeLengthPerChan + (blk) * frameSize + descr * pDevExt->DDR3startAddressFor2ndPingPongBank;
				if (blk < 2) DEBUGP(DEBUG_INFO, "descr %d, pDescriptor->BuffAddr 0x%p => 0x%x", descr, &pDescriptor->BuffAddr, pDescriptor->BuffAddr); // 32-bit address of on_board_memory(DDR3)

				pDescriptor->Usr_Cache = (0x3<<24); // should be defined as a constant somewhere.
				if (blk < 2) DEBUGP(DEBUG_INFO, "descr %d, pDescriptor->Usr_Cache 0x%p => 0x%x", descr, &pDescriptor->Usr_Cache, pDescriptor->Usr_Cache);

				pDescriptor->VSize_Stride = ((pDevExt->VSize)<<19) | (inHSizeByBytes); //0x02000080
				if (blk < 2) DEBUGP(DEBUG_INFO, "descr %d, pDescriptor->VSize_Stride 0x%p => 0x%x", descr, &pDescriptor->VSize_Stride, pDescriptor->VSize_Stride);
	...
	)
{
	...
	DEBUGP(DEBUG_INFO, "pDevExt ->DataBufferStartAddress %d(0x%x) ", pDevExt->DataBufferStartAddress, pDevExt->DataBufferStartAddress)
	pDevExt->MAXframeLengthPerChan = ACQ_SINGLE_CHANNEL_BUF_CAP;
	pDevExt->DDR3startAddressFor2ndPingPongBank = pDevExt->MAXframeLengthPerChan * 4;// ACQ_SINGLE_CHANNEL_BUF_CAP* S2MM_CHANNEL_PER_BOARD;
