...
//  806   Get DMA Engine Cap      	dma_engine_offset (ULONG)  		DMA_CAP_STRUCT
//  807   Acquisition Config Ctrl   IMG_ACQ_CONF_IOCTL	    none
//  808   Get DMA Performance     	dma_engine_offset (ULONG)  		DMA_STAT_STRUCT
//
...
#define DDR3_IMAGE_2ndPINGPONGBUFFstart          DDR3_SINGLEIMAGE_CHANNEL_CAP * 4 // 0x2800.0000 (was * 6) 
...
typedef struct _BUF_ALLOC_STRUCT
{
	...
	ULONG32     dma_engine_offset;  // DMA Engine number to use
	ULONG32	    AllocationMode;     // Allocation type Flags
	ULONG32		packet_max_size;  	// Maximum Packet Size (Must be dividable into PoolSize) (FIFO mode)
	ULONG32		Length;				// Length of Application buffer or size to Allocate (FIFO mode)
	ULONG32		num_of_descriptors;	// Number of C2S Descriptors to allocate (Addressable)
...
typedef struct _RET_BUF_ALLOC_STRUCT
{
	...
	ULONG32		packet_max_size;  	// Maximum Packet Size
	ULONG32		num_of_descriptors;	// Number of C2S Descriptors to allocate (Addressable)
...
{
	USHORT		dma_engine_offset;          // DMA Engine number to use
