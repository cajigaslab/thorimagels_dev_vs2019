...
void FIRFilter::initializeFilterTable()
{
	for (int i = 0; i <= 8; i++) //max downsample rate 2^8
	{
		...
		int filter1_tap = min(FITLER_MAX_TAP,downsample_rate);
		int freq = 1;
		for (int filter2_tap = FITLER_MAX_TAP; filter2_tap > 0; filter2_tap--)
		{
...
FilterSettings FIRFilter::findFilterSettings(double frequency, double adcSampleRate)
{
	...
	filterSettings.filter1Settings = 0;
	filterSettings.filter2Settings = 0;
		...
		for (int j = 15; j > 0; j--)
		{
			...
			else
			{
				...
				if (diff < min_offset)
				{
					filterSettings.filter1Settings = static_cast<int>(pow(2,i));
					filterSettings.filter2Settings = j + 1;
					min_offset  = static_cast<UINT16>(ceil(diff - 0.5));
				}
			}
		}
		if (isFound) break;
