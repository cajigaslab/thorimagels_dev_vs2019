...
THORDAQ_STATUS ThordaqDFLIM:: ConnectToBoard()
{
    ...
    if ( gDeviceInfo == NULL )
    {
        gDeviceInfo = SetupDiGetClassDevs (	
			(LPGUID)&GUID_DEVINTERFACE_dFLIMLegacyDrv, // Pointor to the GUID of Thordaq
	...
	while ( SetupDiEnumDeviceInterfaces ( gDeviceInfo, NULL, &GUID_DEVINTERFACE_dFLIMLegacyDrv, device_count, &device_interface_data ) )
	{
		++device_count;
	}
    ...
    bool_status = SetupDiEnumDeviceInterfaces ( gDeviceInfo, 
		...
		(LPGUID)&GUID_DEVINTERFACE_dFLIMLegacyDrv,
		gBoardIndex, //
...
THORDAQ_STATUS ThordaqDFLIM::SetImagingConfiguration( IMAGING_CONFIGURATION_STRUCT	imaging_config)
{
	...
	memset(&overlapped, 0, sizeof(overlapped));
    overlapped.hEvent = CreateEvent ( NULL, FALSE, FALSE, NULL );
		//NOTE:  IMG_ACQ_CONF_IOCTL invokes function ImageAcquisitionConfig() in the (legacy) kernel driver
		// DAC waveform data is written directly to DDR3 by the upper level DLL in BuildWaveforms process
		// DAC waveform table (at BAR3 0x10000) is written by DAC_DESC_SETUP_IOCTL
		// IOCTLs IMG_ACQ_CONF_IOCTL and DAC_DESC_SETUP_IOCTL do not exist in the NWL (API) driver
...
LONG ThordaqDFLIM::SetGlobalSettings(IMAGING_CONFIGURATION_STRUCT	imaging_config)
{
	...
	gPtrAcqCtrl->gblCtrl.acq_buf_chn_offset = (ULONG32)DDR3_SINGLEIMAGE_CHANNEL_CAP; // 0x400.0000 is kernel driver constant (64 MBytes)
...
LONG ThordaqDFLIM::SetWaveformPlayback( DAC_CRTL_STRUCT dac_setting, int channel, UINT8 set_flag )
{
	...
	int bits_to_shift = ((channel  % 4) * 16); // each 64-bit RATE register holds 4 DAC channel rates
...
LONG ThordaqDFLIM::LoadDACDescriptors(DAC_CRTL_STRUCT* dac_settings)
{
	...
	for (ULONG64 i = 0; i < DAC_CHANNEL_COUNT; i++)
	{
		...
		SetDAC_DMADescriptors(despTable[i], i, dmaDescpTable.descp, is_set_flag);
		if (!is_set_flag)
		{
			return FALSE; 
		}
...
// was SetDMADescriptors
void ThordaqDFLIM::SetDAC_DMADescriptors(DMADescriptor& dmaDescp, ULONG64 index, ULONG64* dmaDescpTable, bool& status)
{
	...
	}else
	{
		...
		SetDAC_DMADescriptors(dmaDescp,++_dacDescpListIndex,dmaDescpTable,status);
	}
}
...
//WriteMem 3 0x1Cc 4 0x00000000
	...
	///////////////////////////////////////////////////////////////////////////
	// line 38    FMC12x CPLD: 	0x02[REG]	0x00[DATA]	LED off (no connectin)
	///////////////////////////////////////////////////////////////////////////
	// See UM009 FMC12x User Manual, pg. 32-33
	// The "PGOOD" LED on mezz. card must be connected to a signal; there
	// are 5 options:
	// 0x01 -- REFMON of AD9517
	// 0x02 -- LD (lock detect) of AD9517 PLL
	// 0x04 -- STATUS output of AD9517
	// 0x08 -- ADT7411 (Temp sensor and ADC) INT# asserted
	// 0x10 -- IRQ, a combination of the other 4 signals (if any are low, LED is asserted)
