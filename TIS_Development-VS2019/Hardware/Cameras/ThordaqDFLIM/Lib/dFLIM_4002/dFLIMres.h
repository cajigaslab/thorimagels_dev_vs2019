...
#define PACKET_READ_IOCTL				CTL_CODE(FILE_DEVICE_UNKNOWN, 0x830, METHOD_OUT_DIRECT,	FILE_ANY_ACCESS)  // these 2 are the 
#define PACKET_WRITE_IOCTL 				CTL_CODE(FILE_DEVICE_UNKNOWN, 0x831, METHOD_IN_DIRECT,	FILE_ANY_ACCESS)  // API ones we use
...
// DDR3 memory map -- 1st Image PingPong buffer start is 0x0
#define DDR3_SINGLEIMAGE_CHANNEL_CAP  0x0E000000 // 224MB (was 64MB or 0x04000000)
#define DDR3_IMAGE_2ndPINGPONGBUFFstart          DDR3_SINGLEIMAGE_CHANNEL_CAP * 4 // 0x3800.0000 (originally 6 chans allocated) 
#define DAC_DDR3_BUF_OFFSET         DDR3_IMAGE_2ndPINGPONGBUFFstart *2 // i.e. 0x7000.0000 (was 0x5000.0000) start of DAC waveforms
// DAQ DRAM MAPPING
...
// define struct for Legacy CL-GUI testing 
// the SetImagingConfiguration()  (ThorDAQAPIdFLIMSetImagingConfiguration) is ridiculously complicated
// to implement in C#, so use simplified struct for only the critical fields we need.

// ThorDAQAPIdFLIMSetImagingConfiguration() is called by upper level DLL to write TILS Params as needed
typedef struct _CL_GUI_GLOBAL_TEST_STRUCT
{
	UINT32 imgHSize;
	UINT32 imgVSize;
	UINT32 channel;
	...
	UINT32 frameCnt;
	UINT32 frameNumPerSec;
	UINT32 acquisitionMode;
	UINT32 triggerMode;
	UINT32 threePhotonMode;
	UINT32 threePhotonPhaseAlignment;
	UINT32 pixelDelayCnt;
	UINT32 turnaroundTime;
	UINT32 flybackTime;
	double PixelDwell;
} CL_GUI_GLOBAL_TEST_STRUCT;
...
typedef struct _DO_MEM_STRUCT
{
	...
	UINT64	VirtBufAddr;        // Byte starting offset in user's Win32 application buffer
	UINT64	BARbyteOffset;         // Byte starting offset in PCI BAR
...
typedef struct _GLOBAL_IMG_GEN_CTRL_STRUCT
{
	...
	UCHAR		img_scan_mode;
	USHORT		ImagingChan;    // enable mask of four channels, e.g. 0111B = channels 0,1,2 are enabled, 3 disabled   
	...
	ULONG32     acq_buf_chn_offset; // "DataBufferChannelOffset" in kernel struct; DataInterruptLength is 6x
	ULONG32     dbg_test_mode;		//1: Debug Mode is activated 
	ULONG32		acquisitionMode; //  (dFLIM only)  0 dFLIM, 1 Diagnostics (.Mode in legacy kernel)
	UINT8		numPlanes;			// number of planes to acquire per frame
} GLOBAL_IMG_GEN_CTRL_STRUCT, * PGLOBAL_IMG_GEN_CTRL_STRUCT;

// NOTE! we cannot locate references to this in DLL or legacy dFLIM kernel
typedef struct _CH_INTERNAL_IMG_GEN_CTRL_STRUCT
{
	ULONG32	beatsPerFrame;  //  cannot locate references to this in DLL or legacy dFLIM kernel
	USHORT	hor_pix_num;
...
{
	UCHAR	CtrlReg;  // 0x180
	USHORT	phase_offset;      // 0x188
	UCHAR	phase_step;
	USHORT	phase_limit;
} SAMPLING_CLOCK_SUBSYS_STRUCT, * PSAMPLING_CLOCK_SUBSYS_STRUCT;

// (NOTE: ThorDAQ is significantly different than dFLIM - expanded/refedined variables/locations
...
// dFLIM Galvo Waveform Generation Subsystem Register(s)  BAR3 0x240
typedef struct _GALVO_WAVEFORM_GEN_SUBSYS_STRUCT
{
	ULONG64 ctrlReg;
	ULONG64 dacUpdateRate[3];  // 0 is Chan 0-3, 1 is Chan 4-7, 2 is Chan 8-11
	USHORT  dacAmplitude;  
	USHORT  dacStepSize[3]; // dFLIM only
	ULONG64 dacParkValue[3];   // 0 is Chan 0-3, 1 is Chan 4-7, 2 is Chan 8-11
	ULONG64 dacOffset[3];      // 0 is Chan 0-3, 1 is Chan 4-7, 2 is Chan 8-11
	ULONG64 dacChannelMap; // the Analog Output MUX
