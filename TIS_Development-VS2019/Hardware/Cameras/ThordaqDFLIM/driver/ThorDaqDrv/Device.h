...
//#ifndef WIN32   // Linux version ---------------------------------------------
//
//static const ULONG32  BAR_TYPE_MASK       = 0x1;
//static const ULONG32  BAR_TYPE_MEM        = 0x0;
//static const ULONG32  BAR_TYPE_IO         = 0x1;
//static const ULONG32  BAR_MEM_ADDR_MASK   = 0x6;
//static const ULONG32  BAR_MEM_ADDR_32     = 0x0;
//static const ULONG32  BAR_MEM_ADDR_1M     = 0x2;
//static const ULONG32  BAR_MEM_ADDR_64     = 0x4;
//static const ULONG32  BAR_MEM_CACHE_MASK  = 0x8;
//static const ULONG32  BAR_MEM_CACHABLE    = 0x8;
//
...
typedef struct _PACKET_GLOBAL_GEN_CTRL_STRUCT
{
	...
	{
		UCHAR		StopRun_FpgaRev;			// Write only bit0 for stop(0) and run(1), the next 4 bits are for debug 
		UCHAR		r1;
		USHORT		FpgaProgStatus;
		UCHAR		r2[4];
		UCHAR		ImgSyncCtrl;
		UCHAR		r3[7];
		USHORT		ImgHSize;
...
typedef struct _SCANNING_SUBSYS_REG_STRUCT
{
	struct  _LAYOUT_SCAN
	{
		USHORT	SyncCtrl_ResScanPeriod;	// offset 0x00
		USHORT	FrameCnt;
		UCHAR	r1[4];
		UCHAR	AdpllCtrl;
		UCHAR	r2[7];
		USHORT	SyncOffset;				// offset 0x10
...
typedef struct _SAMPLING_CLOCK_GEN_SUBSYS_REG_STRUCT
{
	...
	{	
		UCHAR	CtrlReg;			// offset 0x00
		UCHAR	r1[7];
		USHORT	PhaseOffset;
		UCHAR   r2[6];
		UCHAR	PhaseStep;			// offset 0x10
		UCHAR	r3[7];
		USHORT	PhaseLimit;
	} Layout;
...
typedef struct _GALVO_WAVEFORM_GEN_SUBSYSTEM_REG_STRUCT1
{
	...
	{
		ULONG64 CtrlReg;              // offset 0x00     include the Ctrl and UpdateTimeOffset    
		ULONG64 DACUpdateRate[3];     // offset 0x08
		USHORT  DACAmplitude;         // offset 0x20
		USHORT  DACStepSize[3];       // offset 0x22
		ULONG64 DACParkValue[3];      // offset 0x28
	} Layout;
...
{
	PACKET_GLOBAL_GEN_CTRL_STRUCT			globalGenCtrl;
	PACKET_CHANNEL_GEN_CTRL_STRUCT			channelGenCtrl[4];
	...
	IMG_LUT_STRUCT							imgLutTable[S2MM_CHANNEL_PER_BOARD];
