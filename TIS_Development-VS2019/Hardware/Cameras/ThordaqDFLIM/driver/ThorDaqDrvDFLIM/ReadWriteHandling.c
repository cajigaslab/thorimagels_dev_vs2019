	...
	)
{
	...
	{
		cardAddress = (PUCHAR) pDevExt->BarVirtualAddress[barNum] + 0x10000;
		descpPtr = (PULONG)(pDACDescriptor->descp);
		WRITE_REGISTER_BUFFER_ULONG((PULONG)(cardAddress), &(*descpPtr) ,DAC_DESCP_MAX_LEN * 2);
	}
	...
	)
{
	...
	else
	{
		...
		//{
			...
			{
				WRITE_REGISTER_BUFFER_USHORT((PUSHORT)(cardAddress + i * cardOffset), &(pScanLUT->lut[0]), SCAN_LUT_MAX_LEN);
			}
