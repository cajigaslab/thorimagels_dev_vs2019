	...
	)
{
	...
	pDevExt->FrameSize = (USHORT)pDevExt->HSize * (USHORT)pDevExt->VSize;
	...
	for (i = 0; i < S2MM_CHANNEL_PER_BOARD; i++)
	{
		...
		for(j=0; j < framesPerTrans; j++)
		{
			for(k=0; k < S2MM_DESCRS_PER_BLK; k++)
			{
				...
				pS2mmDesc->NxtDescPtr = (((j + 1)% framesPerTrans) * sizeof(S2MM_DESC_BLK_STRUCT) + sizeof(S2MM_DMA_DESCRIPTOR_STRUCT) * k + (i << 16)); 
				pS2mmDesc->BuffAddr = pDevExt->DataBufferStartAddress + ((i + 0) % 4) * pDevExt->DataChannelLength + (j) * inFrameSize + k * pDevExt->DataInterruptLength; //d
	...
	pDevExt->pBar3Map->globalGenCtrl.Layout.StopRun_FpgaRev = 0x03;
