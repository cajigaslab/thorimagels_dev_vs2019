...
bool ThorTsiTest::instanceFlag = false;

long ThorTsiTest::idCounter = 1;

auto_ptr<ThorTsiTest> ThorTsiTest::_single(new ThorTsiTest());

CritSect ThorTsiTest::critSect;

ThorTsiTest* ThorTsiTest::getInstance()
{
	Lock lock(critSect);
	...
	if(! instanceFlag)
	{
		try
		{
			_single.reset(new ThorTsiTest());
			
			TSITest_DbgPrintFunc(TSI_TEST_DEBUG_LVL, _T("ThorTsiTest Created\n"));
...
long ThorTsiTest::FindCameras(const WCHAR *path, long &numCameras)
{
	Lock lock(critSect);
...
long ThorTsiTest::ReleaseCameras()
{
	//remove any preexisting cameras
...
long ThorTsiTest::GetCameraId(long i,long &id)
{
	long ret;
...
ICamera* ThorTsiTest::GetCamera(long id)
{
	ICamera * camera = NULL;	
...
char *GetParamName (ICamera* CameraToTest, ICamera::Params Param)
{
	...
	switch (Param) {
		...
		case CameraToTest->PARAM_OP_MODE:					ReturnValue = "PARAM_OP_MODE";					break;
		...
		case CameraToTest->PARAM_NUMBER_OF_IMAGES_TO_BUFFER:ReturnValue = "PARAM_NUMBER_OF_IMAGES_TO_BUFFER";break;
		case CameraToTest->PARAM_TAP_INDEX :				ReturnValue = "PARAM_TAP_INDEX";				break;
		case CameraToTest->PARAM_TAP_VALUE :				ReturnValue = "PARAM_TAP_VALUE";				break;
		case CameraToTest->PARAM_TAP_BALANCE_MODE:			ReturnValue = "PARAM_TAP_BALANCE_MODE";			break;
...
void CameraTestMenu (ICamera* CameraToTest)
{
	...
	while (!Done) {
		...
		_cputws( L"\nThorTSI Test - C)onsole, P)arameter, I)mage, Esc key to exit program >" );
...
int main (int argv, char *argc [], char *env)
{
	...
	ThorTsiTest* Tester = ThorTsiTest::getInstance();
	...
	for (int i = 0; i < 10; i++) {
		...
		switch (i) {
			...
			case 0 :
#ifdef _WIN64 
				DLL_Path = L"..\\x64\\Debug\\*.dll";
#else
				DLL_Path = L"..\\Debug\\*.dll";
#endif
				break;

			case 1 :
				DLL_Path = L".\\Modules_Native\\*.dll";
				break;
