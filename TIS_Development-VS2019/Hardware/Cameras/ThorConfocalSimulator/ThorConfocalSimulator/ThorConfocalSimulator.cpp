// ThorConfocalSimulator.cpp : Defines the exported functions for the DLL application.
//
...
#endif
{
	...
	_galvoEnable = 0;
	_clockSource = 1;
	_pulseMultiplexingEnable=0;
	_pulseMultiplexingPhase=1;
	...
	_horizontalImageFlip=FALSE;
	_verticalImageFlip=FALSE;

	_pMemoryBuffer = NULL;

	_pDetectorName = L"ResonanceGalvoSim";

	_rawSaveEnabledChannelsOnly = FALSE;

	_rotationAngle = ICamera::LSMGRRotationAngle::DEG_0;
...
long ThorLSMCam::SelectCamera(const long camera)
{
	...
	try
	{
		auto_ptr<ThorConfocalSimulatorXML> pSetup(new ThorConfocalSimulatorXML());

		pSetup->GetConnection(simPath, _rotationAngle, _imageUpdateIntervalMS);
	...
	case ICamera::PARAM_LSM_TYPE:
		{
			...
			paramMax = ICamera::GALVO_RESONANCE;
	...
	case ICamera::PARAM_LSM_AREAMODE:
		{
			...
			paramMax = LINE;
			paramDefault = SQUARE;
			paramReadOnly = ICamera::LSMGRRotationAngle::DEG_0  == _rotationAngle || ICamera::LSMGRRotationAngle::DEG_180 == _rotationAngle ? FALSE : TRUE;
	...
	case ICamera::PARAM_LSM_FLYBACK_CYCLE:
		{
			paramType = ICamera::TYPE_LONG;
			paramAvailable = TRUE;
			paramMin = MIN_FLYBACK_TIME;
			paramMax = MAX_FLYBACK_TIME;
			paramDefault = MIN_FLYBACK_TIME;
			paramReadOnly = FALSE;
		}
		break;
...
long ThorLSMCam::SetParam(const long paramID, const double param)
{
	...
	case ICamera::PARAM_LSM_PIXEL_X:
		{
			...
			if ((param >= MIN_PIXEL_X) && (param <= MAX_PIXEL_X / factor))
				if (static_cast<long>(param) % 16 == 0)
				{
					...
					if (ICamera::LSMGRRotationAngle::DEG_0  != _rotationAngle && ICamera::LSMGRRotationAngle::DEG_180 != _rotationAngle)
					{
						_areaMode = AreaMode::SQUARE;
					}
	...
	case ICamera::PARAM_LSM_PULSE_MULTIPLEXING_ENABLE:
		{
			if((param >= 0) && (param <=1))
			{
				_pulseMultiplexingEnable = static_cast<long> (param);
				ret = TRUE;
			}
		}
		break;
	case ICamera::PARAM_LSM_PULSE_MULTIPLEXING_PHASE:
		{
			...
			{
				_pulseMultiplexingPhase = static_cast<double> (param);
				ret = TRUE;
			}
		}
		break;
	...
	case ICamera::PARAM_LSM_AREAMODE:
		{
			if((param >= SQUARE) && (param <= LINE) && (ICamera::LSMGRRotationAngle::DEG_0  == _rotationAngle || ICamera::LSMGRRotationAngle::DEG_180 == _rotationAngle))
			{	
	...
	case ICamera::PARAM_LSM_FLYBACK_CYCLE:
		{
			_flybackCycle = static_cast<long>(param);
			ret=TRUE;
		}
		break;
	case ICamera::PARAM_LSM_MINIMIZE_FLYBACK_CYCLES:
		{
			_isMinumumFlyback = static_cast<long>(param);
			ret=TRUE;
		}
		break;
	...
	case ICamera::PARAM_LSM_VERTICAL_SCAN_DIRECTION:
		{
			_verticalImageFlip=static_cast<long>(param);
			ret=TRUE;
		}
		break;
	case ICamera::PARAM_LSM_HORIZONTAL_FLIP:
		{
			_horizontalImageFlip= static_cast<long>(param);
			//_horizontalImageFlip= TRUE;
			ret=TRUE;
		}
		break;
	case ICamera::PARAM_LSM_TWO_WAY_ZONE_1:
...
long ThorLSMCam::GetParam(const long paramID, double &param)
{
	...
	case ICamera::PARAM_LSM_AREAMODE:
		{
			...
			{
				param = ICamera::LSMAreaMode::SQUARE;
			}
		}
		break;
	case ICamera::PARAM_LSM_GALVO_ENABLE:
	...
	case ICamera::PARAM_LSM_MINIMIZE_FLYBACK_CYCLES:
		{
			param = _isMinumumFlyback;
		}
		break;
	case ICamera::PARAM_LSM_VERTICAL_SCAN_DIRECTION:
		{

			param=_verticalImageFlip;
		}
		break;
...
void ThorLSMCam::FillImageWithData(GenericImage<unsigned short>& image, int chanIndex)
{
	...
	for(int i=0; i < image.getNumChannels(); i++)
	{
		...
		long size =  static_cast<long>(_simulatorData[chan].size());
...
long ThorLSMCam::ManipulateImage(GenericImage<unsigned short>  * image, long width, long height, ICamera::LSMGRRotationAngle rotationAngle)
{
	unsigned short* intermediateBuffer = new unsigned short[width*height];
	...
	for(int chan=0; chan<image->getNumChannels(); chan++)
	{
		if(image->isChannelEnabled(chan))
		{
			...
			{
			case ICamera::LSMGRRotationAngle::DEG_90:
				{
					IppiSize size;
					...
					ippiDll->ippiRotate_16u_C1R(intermediateBuffer, size, stepSrc, roiSrc, pData, stepDst, roiDst, angle, xOffset, yOffset, IPPI_INTER_LINEAR);
				}
				break;
			case ICamera::LSMGRRotationAngle::DEG_180:
				{
					IppiSize size;
					size.width = width* sizeof(unsigned short);
					size.height = height * sizeof(unsigned short);
				...
				break;
			case ICamera::LSMGRRotationAngle::DEG_270:
				{
					IppiSize size;
					size.width = width * sizeof(unsigned short);
					size.height = height * sizeof(unsigned short);
...
void ThorLSMCam::PrintMessage(GenericImage<unsigned short>& image, double printTime)
{
	...
	for(int chan=0; chan<image.getNumEnabledChannels(); ++chan)
	{
		...
		//=== Fill Flyback Cycles ===
		std::memset(message, 0, sizeof(message));
		if (0 == _isMinumumFlyback)
		{
			wsprintf(message,L"FC: %d FASTEST: off",_flybackCycle,_isMinumumFlyback);
		}
		else
		{
			wsprintf(message,L"FC: %d FASTEST: on",_flybackCycle,_isMinumumFlyback);
		}
		PrintText(imageBuffer, 10, 450, message, chan);

		//=== Fill Flip Vertical and Flip Horizontal ===
		std::memset(message, 0, sizeof(message));
		if (0 == _horizontalImageFlip)
		{
			if (0 == _verticalImageFlip)
			{
				wsprintf(message,L"FH: N FV: N");
			}
			else
			{
				wsprintf(message,L"FH: N FV: Y");
			}
		}
		else
		{
			if (0 == _verticalImageFlip)
			{
				wsprintf(message,L"FH: Y FV: N");
			}
			else
			{
				wsprintf(message,L"FH: Y FV: Y");
			}
		}
		PrintText(imageBuffer, 10, 490, message, chan);
	}
}
