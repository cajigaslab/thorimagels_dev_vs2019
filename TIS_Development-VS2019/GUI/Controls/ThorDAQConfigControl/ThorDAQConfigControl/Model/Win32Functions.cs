...
namespace ThorDAQConfigControl.Model
{
    ...
    public struct CARD_CONFIG_INFO
    {
        ...
        public uint PCIVendorDeviceID;	// (originally "PCIExpressCoreVersion" number)
    ...
    public unsafe struct BOARD_INFO_STRUCT
    {
        ...
        public byte DriverVersionBuildNumber;
        /// <summary>User Version number (optional)</summary>
        public uint UserVersion;       // the Xilinx FPGA version
        public uint PCIVendorDeviceID; // was "PCIExpressCoreVersion"
    }
    ...
    public sealed class Win32
    {
        ...
        public unsafe static uint ThorDAQAPIBindBoard(UInt32 board, ref BOARD_INFO_STRUCT BoardInfo)
        {
            // get the data in unsafe format
            uint status = ThorDAQAPIBindBoard(board);

            if (status == STATUS_SUCCESSFUL)
            {
                status = ThorDAQAPIGetBoardCfg(board, ref BoardInfo);
            }
            return status;
        }
        ...
        public extern static uint ThorDAQAPIBindBoard(UInt32 board);
        ...
        public extern static uint ThorDAQAPIDisconnectFromBoard(UInt32 board);

        /// <summary>
        /// GetBoardCfg - urrent Breakout Box status, Legacy or 3U BOB, CPLD version if 3U
        /// </summary>
        /// <param name="board">Instance number (0 based)</param>
        ...
        public extern static uint ThorDAQAPIGetBoardCfg(UInt32 board,
                          ref BOARD_INFO_STRUCT Board);

        /// <summary>
        /// GetBOBstatus - Calls the driver to get the board config (including PCI header).
        /// </summary>
        /// <param name="board">Instance number (0 based)</param>
        /// <param name="Board">Board Config structure to return data</param>
        /// <returns>Status of the operation</returns>
        [DllImport(".\\Modules_Native\\thorDAQ.dll", EntryPoint = "ThorDAQAPIGetBOBstatus")]
        public extern static uint GetBOBstatus(UInt32 board,
                          IntPtr status, UInt32 length);
        ...
        // Below code are moved from other files
