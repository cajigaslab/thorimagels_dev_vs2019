...
namespace Validations
{
    ...
    public class Validation : ValidationRule
    {
        ...
        public static readonly DependencyProperty SeparatorProperty = 
            DependencyProperty.RegisterAttached("Separator", typeof(string), typeof(Validation), new UIPropertyMetadata(null, OnValidationTypeChanged));
        ...
        public Validation(object owner, Type validationType, double maxValue, double minValue, bool noEmptyValue, string separator, Collection<ValidationError> errors)
        {
            Owner = owner;
            ValidationType = validationType;
            MaxValue = maxValue;
        ...
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            ...
            try
            {
                ...
                sList[0] = value.ToString();

                // Try converting with the validation Type
                if (ValidationType != null)
                {
                    //try parse by defined separator, then validate based on type
                    if (null != Separator)
                    {
                        sList = Regex.Split(value.ToString(), Separator);
                        int iVal = 0;
                        double dVal = 0;
                        foreach (string sItem in sList)
                        {
                            if (typeof(Int32) == ValidationType)
                            {
                                if (!Int32.TryParse(sItem, out iVal))
                                    throw new Exception();
                            }
                            else if (typeof(double) == ValidationType)
                            {
                                if (!Double.TryParse(sItem, out dVal))
                                { throw new Exception(); }
                            }
                        }
                    }
                ...
                foreach (string sItem in sList)
                {
                    if (double.TryParse(sItem, NumberStyles.Any, cultureInfo, out numericValue))
                    {
        ...
        private static void OnValidationTypeChanged(DependencyObject obj, DependencyPropertyChangedEventArgs args)
        {
            ...
            element.Loaded += (s, e) =>
            {
                ...
                var validation = new Validation(obj, GetValidationType(obj), GetMaxValue(obj), GetMinValue(obj), GetNoEmptyValue(obj), GetSeparator(obj), GetErrorOut(obj));
